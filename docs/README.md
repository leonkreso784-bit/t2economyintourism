...
| [CREATE_BACKEND_SPEC.md](CREATE_BACKEND_SPEC.md) | **▶ AKTIVNO (v3, vizija POTVRĐENA) — osobni PRIVATNI UGC-graditelj gradiva „od nule"** — korisnik slaže vlastito ugniježđeno stablo (`nodes`) i u study-čvorovima gradi gradivo (`node_content`) postojećim editorom; tvrda owner-RLS, upis samo kroz RPC-ove; **F1 ✅ (staging, §9 gate 51/51) · F2 ✅ „Moji materijali" (§10) · F3 ✅ editor u čvoru (§11, 9/9)** — slijedi **F4 polish+E2E**; odluka = ADR-024. ⚠️ Spec §11 nosi i **2 blokatora za F5** (upload slike traži `is_admin()`; bucket je javno čitljiv) |
