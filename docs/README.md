...
| [CREATE_BACKEND_SPEC.md](CREATE_BACKEND_SPEC.md) | **▶ AKTIVNO (v3, vizija POTVRĐENA) — osobni PRIVATNI UGC-graditelj gradiva „od nule"** — korisnik slaže vlastito ugniježđeno stablo (`nodes`) i u study-čvorovima gradi gradivo (`node_content`) postojećim editorom; tvrda owner-RLS, upis samo kroz RPC-ove; **F1 ✅ (staging, §9 gate 51/51) · F2 ✅ „Moji materijali" (§10) · F3 ✅ editor u čvoru (§11, 9/9) · F4 ✅ privatne slike + „obriši sekciju" + puni E2E (§12–§13)** — slijedi **F5 = PROD uz Leonov izričit OK**; odluka = ADR-024. ✅ Oba blokatora iz §11 (S1 admin-only upload · S2 javno čitljive slike) **riješena u §12** — bucket `node-images`, privatan, owner-scoped, potpisani URL-ovi |
