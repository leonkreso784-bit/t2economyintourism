...
- **⬜ F4.3c (sljedeće, NAJRIZIČNIJE — prvi pravi write iz preglednika):** svaka kartica dobije „uredi" → edit question/answer u `<sokrat-modal>` formi → JSON Schema validacija → spremi (read-modify-write blob u `subject_content`, RLS write pod admin JWT-om) → verzija (auto trigger) → toast → live re-render. **Test end-to-end traži pravu admin-sesiju** (Playwright nema login) → verifikacija na preview-u (Leon prijavljen) ili stub.
