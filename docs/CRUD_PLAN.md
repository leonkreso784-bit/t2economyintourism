...
> **Status (2026-07-08):** ▶ U TIJEKU. F4.1/4.2 (identitet+write-path+verzioniranje) ✅ · F4.3a/b (detekcija+viewer) ✅ · **F4.3c-1 (prvi write iz preglednika) ✅ ŽIVO VERIFICIRANO** (edit persistira u bazu + verzija + revert) · **Playwright LOGIN (storageState) ✅ — pozitivan admin-put automatiziran** (`test:authed` 3/3 + CI `authed` job). Sve na grani `foundation/f4` (PREVIEW, NIJE produkcija). **Sljedeće: F4.3c-2** (propagacija u finalni). Plan-ugovor za **F4 (custom Admin CRUD)** — uređivanje sadržaja
