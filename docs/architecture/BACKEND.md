...
(predmeti s catalog `content.dataFormat:'json'` — **danas SVI**, a da nije tako, pao bi
`npm run export:json -- --check` u preflightu) → `.js` fallback**. *(Ovdje je do 2026-08-29 stajalo
„18/18"; predmeta je u međuvremenu 24. Pokrivenost drži gate, ne ova rečenica.)* Loader: `_loadSubjectFromJson()` u
...
- **`public.profile_identity`** (F2/3b, 2026-09-09) — `user_id → auth.users`, `display_name`, `bio`, `avatar_path`, `cover_path`. **Owner-RLS samo za čitanje; nijedna upisna politika** — jedini put je **`set_profile_identity`** (SECURITY DEFINER, `auth.uid()`, granice 60/280 provodi baza). ⚠️ **Ime NAMJERNO nije u `profiles`:** pravo na upis vlastitog imena ondje traži UPDATE-politiku, a `profiles` nosi `role` iz kojeg čita `is_admin()`; F7 uz to traži JAVNO čitanje profila, a javna politika nad `profiles` odala bi tko su administratori. `user_metadata.display_name` ostaje **izvedeni preslik za gornju traku** (JWT, bez kruga prema bazi). SQL: **`supabase/f2-profile-identity.sql`** (idempotentan; **na PROD-u od 13.09.**). Brana: `tests/profile-identity.authed.spec.js`.
- **Slike profila (F2/2 cigla 1, 2026-09-13, `supabase/f2-profile-images.sql`):** bucket **`profile-images` `public=true`** (Leon 09.09.: avatar postoji da bi ga se vidjelo; potpisani URL ne bi išao na CDN) — putanja `<auth.uid()>/<avatar|cover>/<uuid>.<ext>`, 5 MB, png/jpeg/webp (bez gif/svg). **Javno je samo čitanje po točnom URL-u**; 4 politike `to authenticated` s vlasničkim prefiksom (InitPlan oblik) → upload, listanje i brisanje samo svoje, `anon` ništa. Putanja u `profile_identity` ulazi **samo kroz `set_profile_image(p_kind, p_path)`** (SECURITY DEFINER): odbija tuđi prefiks i mapu druge vrste (`image_not_owned`), nepostojeći objekt (`image_not_found`), nepoznatu vrstu; dira SAMO traženi stupac. Stari objekt briše klijent kroz Storage API (SQL-brisanje ostavlja S3-siroče). **`delete-account` čisti OBA osobna bucketa** (`PERSONAL_BUCKETS`) — izmjereno 13.09.: bez toga `deleteUser` NE pada, nego avatar obrisanog korisnika ostane **javno dostupno siroče** (GDPR-rupa). Brane: `tests/profile-images.authed.spec.js` (3 testa, crveno→zeleno) · `npm run test:delete-account` T5 (oba bucketa prazna). **na PROD-u od 13.09.** (SQL Leon u SQL Editoru, funkcija kroz MCP, klijent `61c39dd`).
- **Temelj mreže (BACKLOG §🌐 A1–A3, 2026-09-14, `supabase/f2-temelj-mreze.sql`)** — ⚠️ **SAMO STAGING; PROD čeka Leonov OK
  (SQL PRIJE klijenta).** `profile_identity` + **`handle`** (`^[a-z0-9_]{3,20}$`, unique; obični `text` jer pravilo drži mala
  slova) · `handle_changed_at` · **`visibility`** (`private|link|public`, zadano `private`; **nijedna politika ga još ne čita** —
  F7 dodaje JEDNU javnu SELECT-politiku filtriranu njime, nikad nad `profiles`). **`reserved_handles`** (RLS bez politika,
  bez grantova — čita ga samo RPC). Upis imena SAMO kroz **`set_profile_handle(p_handle)`** (SECURITY DEFINER; normalizira;
  kodovi `handle_invalid|reserved|taken|cooldown`; 30 dana između promjena, prvo slobodno; `for update`). **Kvota:** INSERT-
  politika `profile-images` traži `< 20` objekata kroz **`profile_images_count_mine()`** (SECURITY DEFINER, bez argumenta,
  EXECUTE ostaje `authenticated`-u jer politiku izvršava pozivatelj). ⚠️ Podupit nad `storage.objects` UNUTAR politike nad
  `storage.objects` = `42P17` rekurzija, a Storage to javlja kao „schema invalid" — srušio je SVE uploade (izmjereno).
  Brana: `tests/temelj-mreze.authed.spec.js` (6, crveno→zeleno).
- **Preslici u `user_metadata` (JWT, za gornju traku bez kruga prema bazi; izvor istine je uvijek tablica):** `display_name`
  (F2/3b) · **`avatar_path`** (F2/1 ③ — zrcali klijent `SokratProfileImages.mirrorAvatar`, `loadIdentity` popravlja razliku;
  traka propušta samo `<moj-id>/avatar/<ime>`) · **`theme`** (F2/1 ①, `auto` izričito; preuzimanje s uređaja samo pri ulasku).
