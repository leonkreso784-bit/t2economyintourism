...
| Tourism Economics 2 | `te2-hr` | S3 | 🟢 **LIVE (PR #4 mergean na PROD 2026-07-26, `7fb2d61`)** | **Saša: M1 7 kat/51 fc/44 quiz/36 fill + M2 7 kat/43 fc/41 quiz/35 fill; Final=Object.assign 15 kat (+examPractice)** — 3. autorski HR predmet (HR skripta odstupa od EN `te2`); ≤200 znak. (max 195), 0 ćirilice. **Prvi HR year-2 predmet.** Opus lead-review (scope/CI/model/struktura zeleno; činjenice=Sašina domena) + Leonovo dopuštenje → local `--no-ff` merge; Vercel Production SUCCESS. File-served (nije u Supabase). |
