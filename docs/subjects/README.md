...
| Tourism Economics 2 | `te2-hr` | S3 | 🟡 **Saša izvijestio GOTOV (2026-07-23), čeka rebase→PR→lead-review** | **Saša: 13 kat / 84 fc / 77 quiz / 65 fill (+examPractice)** — 3. autorski HR predmet (kao `sit-hr`, HR skripta odstupa od EN `te2`); ≤200 znak., 0 ćirilice, gateovi/smoke zeleni po njemu. **Prvi HR year-2 predmet.** Platformski blocker (`browse.spec` year-2 scoping) RIJEŠEN na main `388e3c5` (2026-07-23-d) → CI mu je sad zelen. NIJE još mergean. |
