...
- **`my-materials.authed.spec.js`** (F2 osobni UGC-graditelj; **5 testova, WRITE — SAMO sa STAGING_\***): stablo se crta ugniježđeno + chevron + **korisnički naziv ESCAPAN** (XSS-granica) · prazno stanje · dodaj folder→gradivo unutra→preimenuj (inline unos, Enter/Escape) · obriši uz potvrdu→„Vrati obrisano" · **povuci ⠿** (gnijezdi u folder / vrati na korijen / ciklus odbijen). Svaki test čisti za sobom (`delete_node`). ⚠️ Dvije zamke naučene: (1) fiksni cookie-banner presreće pointer-evente na dnu → `localStorage 'sokrat-cookie-consent'='denied'` u `addInitScript`; (2) app ima `scroll-behavior: smooth` → `scrollIntoView` animira i `boundingBox()` izmjeri krivo → `addStyleTag` gasi glatko klizanje SAMO u testu.
