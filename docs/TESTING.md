...
- [ ] `npm run test:responsive` → pokreće Playwright (4 iPhone profila):
  ...
  - `dual-read.spec.js` — **JSON dual-read (F2 2A.3):** predmet s `content.dataFormat:'json'` (pilot `sit`) učita study iz `data/json` a NE iz `.js`; **shadow-ekvivalencija** (JSON-učitan `window.sitM1` === `.js`-učitan, bajt-u-bajt); JSON blokiran → `.js` fallback. **Supabase blokiran** u testu (determinizam — inače bi DB shadowao JSON put).
