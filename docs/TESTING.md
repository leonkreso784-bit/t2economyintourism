...
- [ ] `npm run test:responsive` → pokreće Playwright (4 iPhone profila):
  ...
  - `components.spec.js` — **UI-primitivi / Web Components (F2 2D):** `<sokrat-toast>` (registracija + `showToast()` prikaz/auto-hide) · `<sokrat-modal>` (registracija + a11y + open/close stanje + ESC + backdrop-klik) · learn image-viewer (`#imageModal`) otvara/ESC-zatvara/čisti sliku preko `<sokrat-modal>`. **NB:** fokus-management NIJE gate-an (touch-profili ne fokusiraju tapom → verificiran ručno/scratch; `aria-modal=true` deklarativni signal).
