...
- [ ] Playwright uključuje **`tests/sw.spec.js`** (Service Worker: registracija/kontrola nakon reloada + **offline app-shell load** iz keša). **App-testovi imaju `serviceWorkers:'block'`** (u `playwright.config.js` `use`) → deterministički, SW ne presreće `page.route`/fetcheve (inače lomi npr. dual-read). SW se testira IZOLIRANO — samo `sw.spec.js` ima `test.use({ serviceWorkers: 'allow' })`. *(F3 3A)*
