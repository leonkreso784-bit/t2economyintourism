...
1. **⛔ „Povijest učenja" je naslov iznad praznine — i uvijek je bio.** `#historyList` u
   `index.html` nosi komentar `<!-- Generated by JS -->`, ali u `js/**` **ne postoji nijedan
   zapis koji u njega išta piše**; `git log -S historyList` pokazuje da ga nije bilo ni prije
   podjele `app.js` na module. Korisnik na svakom otvaranju stranice napretka vidi naslov bez
   sadržaja. **Dvije mogućnosti, obje su Leonove:** izvesti povijest učenja (podaci postoje —
   `quizScores`, naučene kartice, vrijeme), ili **maknuti odjeljak**. Dok stoji, tri CSS pravila
   (`.history-item`, `:last-child`, `.date`) su mrtva i namjerno ostavljena — brisanje stila ne
   bi riješilo prazan odjeljak, a buduću izvedbu bi ostavilo neoblikovanom.
   ⚠️ Ovo se **slaže s gornjim Leonovim sudom**: „kretanje kroz vrijeme" je upravo ono što bi
   povijest učenja dala, pa je moguće da odgovor nije brisanje nego da je to prva karika.
