...
- **F4 🟡 U TIJEKU — dosadašnje cigle 🚀 DEPLOYANE NA PRODUKCIJU 2026-07-13 (`5d24a96..79f17c7`)** — custom Admin CRUD (= B9/B10 gore): F4.1–4.4 ✅ + draft→objavi staza (`EDITOR_PLAN.md`; U1–U4 ✅ + U-UX ✅ DEPLOYANO → smjer C „Tok" u `EDITOR_UX.md`; **U6 strukturne ops 🔄 U TIJEKU** — grana `feature/u6-structural-ops`, ops-sloj + add-UI + kategorije-UI (U6d) ✅, brisanje/preslagivanje STAVKI čeka DB id-resync); source-of-truth flip = U9+/F4.6 — **dizajniran UGC-spreman, ali student-upload tek nakon F6** (ADR-018). **F5⬜** SRS · **F6⬜** pred-UGC sigurnost (CSP/DOMPurify/moderacija) → **UGC** → tek onda nazad na sadržaj.
