...
| **U8.5** ⏳ | **Media/strukturni blokovi + boje** — dijeli se u pod-korake (obrazac = forma-polja umjesto contenteditable). **U8.5a ✅ 2026-07-22 (`576d73b`):** **slika** = add-tip u ＋ + uredljiva forma (`<input data-be-mfield>` src/alt/caption) + živi preview kroz JEDAN renderer; `change`→patch iz svih polja→`updateLearnBlock`→osvježi samo `.be-media__preview`; unit 35, authed 16/16. **U8.5b ✅ 2026-07-22 (`05b88f8`):** **video** = add-tip + forma (1 polje YouTube link/ID) + facade preview (`mediaPreviewHtml` dispatcher po tipu); sprema `url` raw, renderer izvlači 11-znak ID; unit 37, authed 17/17. **U8.5c ✅ 2026-07-22 (`5003018`):** **formula (KaTeX)** = add-tip + forma (`tex` polje + „veliki blok" prekidač `data-be-mcheck="display"`) + živi preview; RAZLIKA od slike/videa: renderer izbaci `\[tex\]` kao TEKST → `renderMath()` tipografira nakon umetanja (scope = samo `.be-media--formula` preview, `draw()`+change-handler; contenteditable netaknut); change-handler proširen za checkbox-boolean patch; unit 41, authed 18/18. **SLIJEDI:** d=tablica (grid) · e=resize-ručka slike + callout-varijanta · f=boje sekcija + nasljeđivanje (§4). | SREDNJI | authed + validate:schema v2 |
...
| **U8.7** | **Upload slika — drag-drop + Supabase Storage** (Leonova ideja 2026-07-22, „slike iz dokumenata"; konkretizira otvorenu odluku „upload slika/storage"). Bucket + **RLS** (admin-write / public-read; per-user tek za UGC/H2) · **client-side resize/kompresija** (free-tier 1 GB) · **SVG BLOKIRAN** (nosi skripte; `safeUrl` ga već odbija) · drag-zona u **inspektoru** → uploadani javni URL puni `image.src` (blok se NE mijenja — samo novi izvor za `src`). **Redoslijed: nakon U8.5 media.** Podatak≠kod (ADR-018): slika = DATA. | SREDNJI (prvi storage-write put) | authed + storage-RLS test |
| **U8.8** | **`chart` blok — Chart.js facade** (Leonova ideja 2026-07-22). NOVI kurirani blok = **DATA** (`{chartType, labels, datasets, opcije-iz-whitelist}`, serijalizira se čisto — **≠ vježbe/BUG-012** jer nema `generate()` koda); render kroz JEDAN renderer uz **config-whitelist = sigurnosna granica** (ne sirovi Chart.js opcije). Chart.js **vendoran** (CDN kao KaTeX/DOMPurify; §5 uvjeti biblioteke). Ulaz: **paste-iz-Excela (TSV→data) PRVO** (bez teške `xlsx` lib) → **in-app data-builder poslije** (Leonova ideja 3: „grafovi u Sokratu"). Cilj: kvantitativni predmeti (ekonomija/statistika). **Zasebna os — svjesno dodana, NE rastvara U8.5.** | VELIK (nova biblioteka + blok-tip) | authed + validate:schema v2 + render-XSS |

**Otvorene odluke (padaju u ciglama):** ~~upload slika (storage)~~ → **planirano = U8.7** · undo/redo UI (op-stog postoji) · mobilni layout (inspektor <1020px). Svaka cigla mjerena prema mockupu C; grana `feature/u6-structural-ops`, prod netaknut do izričitog deploy-OK-a.
