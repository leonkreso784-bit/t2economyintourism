...
| U8 | **Editor-spike** → odluka biblioteke → **blok-editor** + custom UI-jevi (kartica-flip, quiz-builder, fill-marker, boje). **U8-prep ✅ 2026-07-18:** `admin.js` (1391 LOC) rascijepljen → jezgra 735 + **`admin-editors.js` 670** (čista seoba 5 modal-editora, nula promjene logike; klasične skripte dijele scope; authed 11/11 nula-regresije) → novi U7/U8 kod ide u VLASTITE datoteke, jezgra više ne buja. **Redoslijed: U7 PRIJE U8** (blok-editor uređuje/preview-a U7-ov blok-model + renderer). | ⬜ (prep ✅) |
...
|---|---|---|---|
| **U7a** | **Meta-safe runtime (bivši U2b — SADA je razlog stigao):** `getCategories(content)` helper (jedna rana definicija, global) koji filtrira meta-ključeve (`schemaVersion`/`composedOf`/…) → ožičen u svih **10** `Object.keys` mjesta. Time top-level `schemaVersion` više ne ruši iteracije. `schemaVersion` = marker (odsutan/1 = v1 legacy · 2 = blokovi); legacy se NE stampa masovno. | NIZAK (no-op na sadašnjim podacima; ali dira 5 study-modova) | unit (getCategories s/bez meta-fixture) · **smoke** (study modovi identični — isti gate koji je uhvatio U2a pad) · verify/typecheck |
| **U7b** | **JEDAN renderer, izolirano:** novi `js/blocks-renderer.js` — `renderBlocks(blocks)` → SIGURNI HTML; svaki tip (`heading/paragraph/list/callout/image/video/table/formula/legacy-html`) s ESCAPANIM poljima. Vendoran+pinnan **DOMPurify** (`<script>`, no-build etos kao supabase-js/KaTeX) za `legacy-html`. **YouTube-builder** (validiraj video-ID → `youtube-nocookie` iframe, klik-za-učitavanje, consent-gated; autor NIKAD ne unosi iframe). `formula` → postojeći `js/math.js`. **NIJE još ožičen na study** (nema v2 podataka → 0 utjecaja na studente). | SREDNJI (nova sigurnosno-kritična logika + nova vendorana ovisnost) | teški unit: svaki tip → očekivani izlaz · **XSS-fixtures** (`<script>`/`onerror`/`javascript:` u poljima → escapano/uklonjeno) · legacy-html → sanitiziran · pure-fn (lako izolirati) |
| **U7c** | **Flip sigurnosne granice:** `learn.js` `renderLearnContent()` → dual-mode: v2 (`learn.blocks`) → `renderBlocks`; v1 (`learn.content`) → `renderBlocks([{type:'legacy-html', html}])`. Time SAV learn ide kroz sanitizirajući renderer (jedan put za study+editor+marketplace). | **NAJVEĆI** (dira ŽIVI student learn-put; DOMPurify smije ostrugati klasu/atribut koji naš CSS/KaTeX koristi) | **parity-harness: 18/18 predmeta learn stari-vs-novi render = identično** (DOMPurify allowlist ugođen na naše tagove/klase/KaTeX-izlaz) · smoke · vizualni spot-check · **fallback:** ako parity ne prođe čisto → v1 ostaje sirov (naš povjerljiv sadržaj), granica se primjenjuje samo na v2 |
| **U7d** | **Schema v2 + validator + dual-read:** JSON Schema prima `learn.blocks` (+ `schemaVersion`); `validate-content`/`validate-schema` validiraju tipove/polja blokova; `export:json` + content-loader dokazano nose blokove (round-trip). | NIZAK-SREDNJI (tooling; ne dira runtime) | validator-testovi · export round-trip · verify |
| **U7e** | **Blok-ops u draftu (data-layer za U8):** `addBlock/removeBlock/reorderBlocks/updateLearnBlock` u `draft-store.js` (idempotentni po id-u, kao U6a; sibling-replay netaknut). Editor (U8) ih kasnije ožičuje. | NIZAK (aditivno na dokazani ops-sloj) | draft-store unit (kao U6a) |
