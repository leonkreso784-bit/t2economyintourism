...
| U8 | **Vizualni editor = Studio iz mockupa** (razrada §12.2, **RE-PLAN 2026-07-20**). **U8-prep ✅ 2026-07-18** (rascjep `admin.js`→`admin-editors.js`). **U8a ✅ 2026-07-20** (`block-editor.js` jezgra; ALI bolt-an na stari admin → Leon presudio „katastrofa" → **ZAOKRET opcija b: Studio-kosti prve, vizual zadnji**; jezgra se zadržava, admin-integracija umirovljuje). **SLIJEDI: U8.1 (Studio-skelet `#editor-page`) → U8.2–U8.6.** | ⏳ (U8a ✅; U8.1 slijedi) |
...
| Cigla | Što | Rizik | Provjera |
|---|---|---|---|
| **U8.1** | **Studio-skelet (struktura, grubo stilizirano):** nova `#editor-page` — topbar+breadcrumb · **STABLO** (fakultet→…→predmet→skripte, iz kataloga) · canvas (naslov+meta+mode-tabovi) · inspektor; ožičen na draft (Uredi→draft, Objavi=RPC). Zamjenjuje „select subject/lesson" dropdowne. | VELIK (nova ljuska) | smoke + authed (nav+draft) |
| **U8.2** | **Learn-pane = blok-editor u canvasu:** `block-editor.js` jezgra preseljena u learn-pane (kvadratići), VIDLJIVA, na pravim kostima. | SREDNJI | authed (blok add/reorder/remove) |
| **U8.3** | **Kartice/kviz/fill = mode-tabovi:** postojeći editori kao paneli u canvasu (＋/✕ modovi; features-flag; prazan mod ≠ greška). | SREDNJI | authed 11/11 + novi |
| **U8.4** | **Inline uređivanje teksta:** contenteditable → `inline runs` (B/I/boja/link plutajuća traka) → `updateLearnBlock`. | SREDNJI (contenteditable→runs) | unit + authed |
| **U8.5** | **Media/strukturni blokovi + boje:** slika/video/tablica/formula/callout/lista · resize-ručka (§5.1-8) · boje sekcija + nasljeđivanje (§4). | SREDNJI | authed + validate:schema v2 |
...
**Status cigli:** **U8a ✅ napravljen** (`4794498`, 07-20; `block-editor.js` jezgra + CSS + admin bolt-on; unit 18/18) → **jezgra SE ZADRŽAVA (U8.2), admin-integracija SE UMIROVLJUJE.** SLIJEDI: **U8.1 (Studio-skelet).**
