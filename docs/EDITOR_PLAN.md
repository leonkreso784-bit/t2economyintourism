...
| U8 | **Vizualni editor = Studio iz mockupa** (razrada §12.2, **RE-PLAN 2026-07-20**). **U8-prep ✅ 2026-07-18** (rascjep `admin.js`→`admin-editors.js`). **U8a ✅** (`block-editor.js` jezgra; bolt-an na stari admin → „katastrofa" → **ZAOKRET opcija b: Studio-kosti prve, vizual zadnji**). **U8.1 ✅ 2026-07-20** (Studio-skelet `#editor-page`: topbar/stablo/canvas/mode-tabovi/inspektor; live-smoke stablo 57 skripti + preview 4 taba, 0 grešaka). **SLIJEDI: U8.2 (blok-editor u learn-pane) → U8.3–U8.6.** | ⏳ (U8a+U8.1 ✅; U8.2 slijedi) |
...
**Status cigli:** **U8a ✅** (`4794498`, 07-20; `block-editor.js` jezgra + CSS + admin bolt-on; unit 18/18) → jezgra SE ZADRŽAVA (U8.2), admin-integracija SE UMIROVLJUJE. **U8.1 ✅ 2026-07-20** (`js/studio.js` + `css/studio.css` + `#editor-page` ruta + `SokratAdmin.studioBridge`; live-smoke: stablo 57 skripti → klik → canvas 4 mode-taba + preview + breadcrumb, 0 grešaka; unit 130/0, admin-regresija 40/40). SLIJEDI: **U8.2 (blok-editor jezgra → learn-pane canvasa).**
