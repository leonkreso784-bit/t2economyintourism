...
- [3E] **a11y prolaz** (tipkovnica/ARIA/kontrast) — pro + SEO. ✅ **3E.1 GOTOVO (2026-07-05, grana `foundation/f3d`; NIJE deployano):** dubinski axe audit (SVI impact-levovi, sve sekcije uklj. flashcards/quiz/fill/progress/legal).
  ...
  **Popravljeno (0 serious/critical ostalo):** (a) **button-name** — flashcard `#btnPrev`/`#btnNext` (samo ikona) → `aria-label` (novi i18n mehanizam **`data-i18n-aria`** + `fc.prev`/`fc.next` en/hr; ikone `aria-hidden`); (b) **select-name** — quiz `#questionCount`/`#quizCategory`/`#quizDifficulty` → `<label for>` povezan;
  ...
  ✅ **3E.2 GOTOVO (2026-07-06, grana `foundation/f3d`; NIJE deployano):** moderate landmarks — **sve 4 stranice sad 100% axe-clean (0 violationa BILO kojeg levela).** `region`: landing `.hero-stats`→`role=region`, `.landing-cta`→`aria-labelledby`, `.landing-nav`/`.landing-footer` + study/browse/profile zaglavlja (ugniježđena u `<section>` → izgubila implicitni banner/contentinfo) → **eksplicitni `role="banner"`/`role="contentinfo"`** (jedna stranica vidljiva odjednom → nema duplikata). `heading-order`: footer `h4`→`h3` (preskakao h2→h4; CSS zadržao veličinu). Sve **atribut-only osim footer tag+CSS** → 0 layout-rizika. Gate: **PUNA Playwright 185/0**, axe 4/4 **potpuno clean**.
  **⬜ deploy uz F3 nastavak** (3D+3E zajedno). **⬜ opcionalno: 3C.2** (auto-bump na Vercel deploy-u).
