...
- **1D — TVRDI kvalitetni gateovi (nadogradnja #1 — razlika „zdravo→brutalno"):** ✅ **GOTOVO + GITHUB-ZELEN 2026-06-29** (run #28386199455, oba joba success).
  - [1D.1] ✅ **Lighthouse CI** (`.lighthouserc.json`, `@lhci/cli`, zaseban CI job `lighthouse` na Linuxu — Windows lokalno ima chrome-launcher `EPERM` na OneDriveu). **Tvrdi budžeti (`error`)**, zasad KONZERVATIVNO: a11y/best-practices/seo ≥ 0.9, performance ≥ 0.6 (3 mjerenja, median). ⏳ **Kalibracija:** stegnuti prema 0.95 kad pročitamo stvarne CI-brojeve (lhci uploada javni report URL u job-log).
