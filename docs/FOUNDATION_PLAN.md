...
> **Status (ažurirano 2026-07-06):** ▶ U TIJEKU. **F1 (reliability rails) ✅ DEPLOYANO · F2 (reusable jezgra) ✅ KOMPLETNA + DEPLOYANO · F3 (performanse) ✅ KOMPLETNA + DEPLOYANO** (jezgra 3C.1+3B+3A 2026-07-05; 3D+3E ff-merge `e39eb1d..b19a641` 2026-07-06, live-verified `20260706003609`). **F4 (Admin CRUD) ▶ U TIJEKU** (grana `foundation/f4` = preview: F4.1 identitet + F4.2 write-path/verzioniranje + F4.3a/b viewer + **F4.3c edit kartice end-to-end (c-1 write + c-2 midterm↔final propagacija)** + **Playwright login (storageState)** gotovi+ŽIVO-verificirani (2026-07-08); **F4.4 tipovi (quiz/fill/learn/kategorije) sljedeće**; ADR-021 + `CRUD_PLAN.md` + `CATALOG_ARCHITECTURE.md`) → F5 (SRS) → F6 (sigurnost) → UGC. Detalji po fazama niže.
