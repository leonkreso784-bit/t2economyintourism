...
| **F2/2** (iza 3b) | **Profilna slika + naslovna** — bucket po obrascu `node-images`: vlasnički prefiks + RLS. ⚠️ **Ide IZA F2/3b** (Leon, 2026-09-09): putanja se upisuje u `profile_identity`, pa bi prije toga završila u `user_metadata` i selila se dvaput. **Otvoreno pitanje za tu ciglu:** privatan bucket + potpisani URL (kao `node-images`) ili javan bucket uz upis samo vlasniku — preporuka je javan, jer avatar postoji da bi ga se vidjelo i F7 ga ionako otvara. | …korisnik stavi svoju sliku i vidi ju odmah, a tuđi prefiks mu je nedostupan |
