...
| **①/3c** pet samostalnih stranica bez ijedne mjere za telefon | Nalaz revizije ①/3b: root ima 8 `*.html`, a **`contact` · `faq` · `odjava` · `privacy` · `terms` nemaju nikakvu mjeru za telefon** — točno razred zbog kojeg je ①/3b i nastao. Uz to ⓪ „sve ekrane" danas znači „koliko ih je u ručnom popisu". Smjer: **nabroji `*.html` s diska**, svaka je ili obiđena ili imenovana uz razlog (kalup `EDGE_BEZ_STRAZE` / `OTVORENO`) | ⓪ pada po defaultu na novoj `.html` datoteci koju nitko nije obišao ni imenovao |
