...
| **①/3b** telefon za stranicu odobrenja | `odobrenje.html` nije ni u jednoj mjeri za telefon. Ide po obrascu koji `tests/helpers/phone-gate.js` već ima za **samostalan dokument** (`editor.html`: `page.goto` + čekanje selektora, ne `navigateTo`). Dva stanja vrijedi mjeriti: **„prijavi se"** (bez sesije; ne traži mrežu jer `getSession()` čita localStorage) i **„Dopusti / Odbij"** (traži podmetnut Auth, kojeg `phone-gate` danas nema) | osnovica `tests/phone-baseline.json` je prazna → brana traži nulu nalaza |
