...
| **F1/10** | **Zoom na dodir** (Leon, 2026-09-05: *„kada se više puta takne na jedno mjesto može se zoomat, to se mora riješit"*). **Dva uzroka:** ① polja za unos **ispod 16 px** — iOS pri fokusu sam zumira; **izmjereno:** na svakoj ruti 11 polja (`.auth-modal__input` 15,2 px · `.cat-search-input` 14,4 px), ostala ≥ 16 · ② Safarijev **dvostruki dodir** — u headless-motorima **nemjerljiv** (24 mjerenja, skala ostaje 1); gasi ga `touch-action: manipulation`, koji repo već drži na `.flashcard` i slijepoj karti, a štipanje i skrol ostaju. Popravak: ① polja ≥ 16 px na dodiru + **deveta tvrdnja `phone-gate`-a** (danas crvena) · ② `manipulation` u `*` resetu i u `legal.css`. `user-scalable=no` odbačen (gasi štipanje). Ide u istu dodirnu seriju s F1/8 ①. Mjere: [BACKLOG.md](../records/BACKLOG.md) §LEONOVI NALAZI F. | …Leon dodirne polje ili dvaput dodirne tekst i stranica **ostane iste veličine** |
