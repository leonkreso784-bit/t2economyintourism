...
| **F3/2** | **Zakucan tekst u datotekama s mehanizmom** — `index.html`, pa redom po težini. ✅ **Cigla 1 (16.09., na grani):** `index.html` 49 → 21 · `editor.html` 5 → 0 · `sokrat-toast.js` 1 → 0 (ukupno 224 → 190). **Pravilo cigle:** mjesto koje JS UVIJEK piše (kartica, kviz, naslovi, toast) ostaje u markupu PRAZNO, ne dobiva `data-i18n` — `applyTranslations()` bi ga na promjenu jezika pregazio (brana: `i18n.spec.js` F3/2, obrnuto provjereno). Novi mehanizam `data-i18n-title`; oznaka prekidača ide kroz ključ `topbar.langCode`, zaseban pisac u `i18n.js` uklonjen. Ime prekidača dvojezično u oba jezika (Leon, 16.09.). ✅ **Cigla 2 (16.09., na grani): slijepa karta** — `index.html` 21 → **0** · `js/blind-map.js` 8 → 0 (ukupno 190 → 161). Poruke iz stanja (koordinate, rezultat) precrtava kuka `renderBlindMapText` u `applyTranslations`; poruka s imenom mjesta ide kao DOM, ne `innerHTML`; zadatak više ne piše „Find:" dvaput. **Red dalje:** `exercises.js` → `profile.js` → `my-materials.js` → `learn.js`/`auth.js` → `progress.js` → … (editorske datoteke su F3/3). **Brana ne vidi, a treba uzeti uz tu datoteku:** `js/auth.js` `aria-label` gumba za prijavu („My profile"/„Sign in", ternarni izraz) · `js/learn.js` engleski rezervni `alt` uvećane slike. | …korisnik prebaci jezik na naslovnici i ne vidi engleski ostatak |
