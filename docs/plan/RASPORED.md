...
| **①/3d** poravnanje samostalnih stranica (Leon, 21.09.: *„naravno da treba sve poravnati"*) | Šest samostalnih dokumenata (odobrenje + 5 pravnih) danas je na iPhoneu **uokvireno** — traka pozadine gore i dolje umjesto boje od ruba do ruba kakvu ima aplikacija. Poravnanje = `viewport-fit=cover` + nadoknada kroz `var(--safe-*)` u `css/legal.css` (⚠️ `env()` samo u `css/variables.css`, brana `check:safearea`). Time `BEZ_IZREZA` ostaje **prazan**, a ①/⑥/⑦/⑦b se na tim stranicama uključuju. | mjera iz ①/3b–c je već napisana: čim dobiju `cover`, ① prijavi logo i prekidač jezika ispod izreza (izmjereno: 14 nalaza) — cigla je gotova kad ih nema, a `BEZ_IZREZA` je prazan |
