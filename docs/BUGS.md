...
- **Lekcija:** **Playwright ne može login na Supabase → testira samo odjavljene/negativne putove.** `admin.spec` je provjeravao samo `isAdmin===false` (točno i dok je detekcija PUKNUTA) pa je bug prošao SVE testove. **Za auth/RLS-gated značajke (CRUD) nužna je PRAVA prijava** (login-skripta / preview). Uz to: **globali deklarirani kao `const` NISU na `window`** — referenciraj ih golo (`typeof X !== 'undefined'`), ne `window.X`. [[live-login-verifies-crud]]
