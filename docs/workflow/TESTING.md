...
- [ ] **`npm run css:diff`** → 0 razlika u prikazu. Uspoređuje IZRAČUNATE stilove u pravom Chromiumu (radno stablo vs `HEAD:styles.bundle.css`) preko 3 širine; tokeni se broje odvojeno od prikaza. **Traži preglednik i port → NIJE u preflightu**; pokreni ga uz svaku ciglu koja dira CSS. U C1 je dokazao 3438 usporedbi / 0 razlika; u C2–C7 dokazuje da se promijenila **samo** ciljana površina. *(C1)*
      ⚠️ **ZNA MJERITI SAMO PROMJENE U CSS-u — presreće stylesheet, a HTML uzima iz radnog stabla.** Premjesti li cigla vrijednost **iz markupa u CSS** (T5: dvoosna veličina heroja se utilityjem ne da napisati), njegova referenca je stranica koja **nikad nije postojala** — novi markup + stari CSS — pa prijavljuje razlike i ondje gdje se ništa nije promijenilo (T5: 46 razlika, uključujući 768 i 1280 px). Dotad se takva cigla dokazuje **pravim A/B-om**: HEAD poslužen iz zasebnog `git worktree`-a na drugom portu, obje verzije sa **svojim** markupom i **svojim** CSS-om (T5: **0 razlika na 768 i 1280**, 22 na 375 i sve namjera). *Gate koji mijenja samo jednu polovicu stranice mjeri stranicu koja ne postoji.* Otvoreno u `BACKLOG.md`; ponovit će se u C4–C7.
...
- [ ] `npm run test:responsive` → pokreće Playwright (4 iPhone profila):
  ...
  - `landing.spec.js` — landing nav, subjects showcase (= broj predmeta iz catalog-a),
    navigacija CTA-ova, overflow guard. **+ T5: potez preko fraze (`.hero-mark`) stoji u JEDNOM retku i ne prelijeva stupac** — mjeri se u **oba jezika** (hrvatsko „četiri načina" je 13 znakova naspram engleskih 9) i **izričito na 320 px**, jer projekti ove suite počinju na 375 a kriterij §2 imenuje 320; obrnuta provjera je pokazala da tvrdnja koja mjeri samo projektnu širinu **prolazi nad kvarom**. Tvrdnja čuva **ishod, ne mehanizam**: `white-space: nowrap` je bio napisan pa obrisan (obrnuta provjera dokazala da nije nosiv — drži ju veličina naslova), a pada vrati li se naslov na 48 px.
...
- **🏗️ STAGING (U1, 2026-07-10):** postoji **`sokrat-staging`** (ref `czljmvigkgiajzjxtndq`, 2. free projekt) = izolirani test-DB. Kad su `STAGING_SUPABASE_URL/ANON/TEST_ADMIN_EMAIL/PASSWORD` u `.env`, **`test:authed` + `rls-check` automatski gađaju staging** (`js/auth.js` `_readSupabaseOverride()` preusmjeri app; `SUPABASE_TARGET=staging` za rls-check) → **write-testovi (edit-pa-revert) sad rade na stagingu, prod audit ostaje čist.** Bez `STAGING_*` → staro ponašanje (prod TEST_ADMIN).
  ...
  - `phone.spec.js` — **TVRDI gate (T0+T1+T4; 10 testova, mjera u `helpers/phone-gate.js`):** **jedina brana koja mjeri TELEFON KAO STRANICU.** Postoji jer je produkcija na 393 px bila neupotrebljiva uz **desetak zelenih gateova** — axe mjeri na **1280 px**, `css:diff` uspoređuje nas **sa samima sobom** (hvata PROMJENU, ne LOŠOĆU), a K3/K4a mjere **kromo**. Pet tvrdnji na **320 · 393 · 430 px** (stvarne visine uređaja, jer se kromo mjeri kao UDIO): ① ništa interaktivno u gornjih **59 px** · ② naše trake ≤ **20 % upotrebljive** visine (otok se ne broji — uređaj ga uzima svakome) · ③ tekst se ne lomi preko 2 retka dok mu **susjed krati** · ④ bar jedna **sadržajna** kontrola **dohvatljiva** bez skrola (`elementFromPoint`, jer cookie-banner ima `z-index: 2147483000`) · ⑤ naslov razine je čitljiv (≤ 1 redak **i** nije odrezan ispod 60 %). Uz to ⓪ **pokrivenost** — brana koja tiho preskoči ekran tvrdi nešto što nije vidjela (BUG-017). Obilazi **i četiri načina učenja** na pravoj lekciji.
    ...
    ⚠️ **OSNOVICA, NE NULA.** Nalazi su planom dodijeljeni ciglama T1–T5, pa bi nula držala cijelu suitu crvenom kroz **pet** cigli — a tada prava regresija u ostalih 400+ testova nestane u šumu. Vrijedi obrazac `check:palette`: **`tests/phone-baseline.json`** drži poznate kvarove (**javno 8, prijavljeno 0** — nakon T5; svih 8 je tvrdnja ④, jer su ①②③⑤⑥⑦⑦b⑧ na NULI). ⚠️ **Ovdje je do T4 pisalo „svi zbog cookie-bannera" i to je bilo netocno** — poruka nalaza ispisuje visinu trake **kad god traka postoji**, ne kad je ona kriva, pa je optužba pročitana iz formata poruke. Izmjereno (svaki ekran i s trakom i bez nje): traka je bila uzrok na **3**, ostatak su `lessons` bez ijedne kontrole (**BUG-032**), `about` s jednom kontrolom na `y ≈ 1500`, i `landing` gdje hero gura vrata ispod pregiba. **Landing je riješen ciglom T5** (vrata na 320 px s `y = 567` na `y = 338`), pa preostalih **8** dijele `lessons` (BUG-032) i `about` (čeka Leonovu odluku). Brana pada **samo na NOVOM** nalazu, a riješeni se ispisuju glasno.
