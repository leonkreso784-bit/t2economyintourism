...
- **Ukupno `test:authed` = 77** (+ `auth-setup` 1), **prebrojano i izmjereno 2026-08-19** — statički zbroj `grep -c "^\s*test("` po `*.authed.spec.js` i runtime-brojka iz suite se poklapaju: studio 15 · my-materials 12 · node-editor 8 · material-authoring 8 · admin-detect 6 · node-images 5 · card-limits 3 · studio-math 3 · **reachability 3** · cascade 2 · a11y 2 · f4-e2e 2 · publish-rpc 2 · studio-chrome 2 · category-ops 1 · item-ops 1. ⚠️ **Ovdje je do 2026-08-19 pisalo „= 20", s razlaganjem iz vremena U8** — brojka je bila **zastarjela za 57 testova**, a razlaganje je navodilo tri pojedinačna broja koji su svi bili krivi. *Zbroj prepisan u prozu ostari tiše od pojedinačnih brojki, jer ga nitko ne provjerava dok ne zatreba.* Write-testovi traže **SEEDAN staging** (`node scripts/seed-staging.js te2`).
