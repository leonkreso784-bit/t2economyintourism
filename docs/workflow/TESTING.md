...
- **`node-editor.authed.spec.js`** (F3 editor u study-čvoru; **9 testova, WRITE — SAMO sa STAGING_\***): prazan čvor → draft-mod · uredi → **`publish_node`** → ponovno učitaj = sadržaj ostao + verzija 1→2 + audit-redak · zastarjeli `base_version` → `publish_version_conflict` (izgubljeni upis odbačen) · klik „Uredi gradivo" → Studio na čvoru (crumb/naslov/panel/„Uredi") · „←" → profil · **prazan čvor → „＋ Nova sekcija" → Objavi → sadržaj u bazi** · druga sekcija ne gazi prvu · `setLesson` gasi node-mod.
...
- **`node-images.authed.spec.js`** (F4 privatne slike; **4 testa, WRITE — SAMO sa STAGING_\***): upload u node-modu ide u `node-images` **pod vlasnički prefiks** (`<uid>/<node_id>/<uuid>`) i vraća **oznaku** `node-img:` · objavljeni payload **zadrži oznaku** (bez `token=` — potpis koji istječe ne smije u bazu) · pri prikazu se oznaka razriješi u **potpisani URL koji stvarno vrati sliku** (uz kontrolu da **nerazriješena oznaka NE proizvede `<img>`** = fail-safe, i da razrješavanje **ne mutira payload**) · **katalog-mod NEDIRNUT** (i dalje javni `lesson-images`). Testovi gađaju **proizvodni** `uploadImage` kroz `window.__beMedia(core)`, ne zaobilaznicu, i **čiste za sobom** (objekt + čvor). RLS/HTTP stranu pokriva `npm run test:storage`.
- **`my-materials.authed.spec.js`** (F2 osobni UGC-graditelj; **13 testova, WRITE — SAMO sa STAGING_\***): stablo se crta ugniježđeno + chevron + **korisnički naziv ESCAPAN** (XSS-granica) · prazno stanje · dodaj folder→gradivo unutra→preimenuj (inline unos, Enter/Escape) · obriši uz potvrdu→„Vrati obrisano" · **povuci ⠿** (gnijezdi u folder / vrati na korijen / ciklus odbijen). Svaki test čisti za sobom (`delete_node`). **Sve su tvrdnje SCOPE-ane na `data-mm-id` čvorova koje test sam stvori** — gate NE SMIJE ovisiti o tome je li račun prazan (3 su testa padala čim je Leon ručno klikao po istom stagingu). Uključuje i 6 rubnih slučajeva: dubina 8 razina bez vodoravnog overflowa · naziv od 120 znakova · dvoklik na „+ Folder" · dvostruki Enter · drop na samog sebe · prebacivanje inline unosa. ⚠️ **Ne tvrdi „prazno stanje" bez čekanja na kraj učitavanja** — spinner koristi ISTI `.mm-state-title`, pa je stari test prolazio NAD SPINNEROM i bio bi zelen i da je učitavanje potpuno slomljeno; `openProfile` sad čeka nestanak `.mm-spin`. ⚠️ Dvije zamke naučene: (1) fiksni cookie-banner presreće pointer-evente na dnu → `localStorage 'sokrat-cookie-consent'='denied'` u `addInitScript`; (2) app ima `scroll-behavior: smooth` → `scrollIntoView` animira i `boundingBox()` izmjeri krivo → `addStyleTag` gasi glatko klizanje SAMO u testu.
