> 🗄 **ARHIVIRANO** (2026-08-07) — ISPUNJEN — Admin CRUD javnog kataloga dovršen kroz EDITOR_PLAN U-slijed.
>
> **Referenca, ne izvor istine.** Što sada vrijedi → [docs/README.md](../README.md).

# CRUD_PLAN — F4 Admin CRUD (Sokrat Study)

> **⚠️ STATUS 2026-08-04: ✅ ISPUNJEN — admin CRUD/editor je funkcionalno gotov** (v. `EDITOR_PLAN.md §12`).
> Ovaj dokument je **REFERENCA za povijest cigli, NE aktivni plan.** Aktivna staza = **`CREATE_BACKEND_SPEC.md` v3**
> (osobni UGC-graditelj; F0–F4 ✅, slijedi F5 = PROD).
> **Pazi na numeriranje:** ovdje „F4" = FOUNDATION-F4 (Admin CRUD za javni katalog), NE CREATE_BACKEND-F4. Tablica u `CLAUDE.md`.

> **Povijesni status (2026-07-14):** ▶ U TIJEKU. F4.1/4.2 (identitet+write-path+verzioniranje) ✅ · F4.3a/b (detekcija+viewer) ✅ · **F4.3c (prva UI cigla — edit kartice end-to-end) ✅ KOMPLETNA + ŽIVO VERIFICIRANA** (c-1 write+verzija+revert; c-2 propagacija midterm↔final) · **Playwright LOGIN (storageState) ✅ — pozitivan admin-put automatiziran** (`test:authed` 4/4 + CI `authed` job). **F4.4 quiz+fill+learn ✅ ŽIVO VERIFICIRANO** (edit persistira+propagira na final+revert; prod 51 red netaknut). **🚀 SVE DEPLOYANO NA PRODUKCIJU 2026-07-13** (ff-merge `5d24a96..79f17c7`, live-verified `20260712180655`).
> **🧭 SMJER PROMIJENJEN (korisnik 2026-07-08/09):** nastavak F4 ide kroz **[EDITOR_PLAN.md](./EDITOR_PLAN.md)** brick-slijed (U1 staging ✅ 2026-07-10 → U2a id-jevi ✅ 2026-07-11 `b490172` → U2.5 ADR-022 placement ✅ 2026-07-11 `b969892` → U3 draft-sloj ✅ KOMPLETAN 3/3 2026-07-12 `281f5e3`+`468e477`+d3 (editori pišu u draft; Objavi-put ŽIVO verificiran na stagingu — MCP: md5==baseline, cv 0→4, final sinkan) → U4 publish-RPC ✅ 2026-07-13 `1e89f99`+`d251e78` (atomično + `base_version`; grana `feature/u4-publish-rpc`; 🚀 DEPLOYAN 2026-07-14) → U-UX ✅ 2026-07-14 (smjer C „Tok" → `EDITOR_UX.md`) → ✅ U6 strukturne ops KOMPLETNE (2026-07-17, grana `feature/u6-structural-ops`: kategorije + stavke — add/edit/reorder/remove; DB id-resync 16 eng. predmeta odrađen → item delete/reorder odblokiran; authed 11/11) → C-vizual U8): CRUD prelazi na **draft→objavi** model; „F4.4-kategorije" se radi kao **U6 strukturne operacije** unutar draft-moda; F4.5 (export/dry-run) i F4.6 (flip) = U9+. Dosadašnji F4.1–F4.4 rad je temelj i ostaje (RLS, verzije, editori se prevezuju na draft). Plan-ugovor za **F4 (custom Admin CRUD)** — uređivanje sadržaja
