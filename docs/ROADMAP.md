...
normalizirano 92→`20260704162056`; **ADR-017**. ✅ **3B CSS bundling GOTOVO** (2026-07-05, grana `foundation/f3`, NIJE deployano): 26 `@import`→1 `styles.bundle.css` (`scripts/build-css.js` + CI drift-gate; `styles.css`=izvor-manifest, index.html→bundle; eliminiran render-blocking waterfall, perf mjeri CI Lighthouse).
✅ **3A.1/3A.2 Service Worker GOTOVO** (2026-07-05, grana `foundation/f3`, NIJE deployano): „Works offline" = istina. `sw.js` (navigacija network-first + fallback shell; asseti SWR; Supabase/CDN network-only; kill-switch; `SW_VERSION` u `npm run bump`) + `sw-register.js` (`updateViaCache:'none'`) + `vercel.json` `/sw.js` no-cache. Copy „Works offline" vraćen. Test `sw.spec.js` (offline load). Regresija SW-vs-test popravljena (`serviceWorkers:'block'` global, SW izoliran u sw.spec). **Playwright 173/0.** **⬜ 3A.3 (update-flow) + deploy → FABLE.** → ⬜ **3C.2** (auto-bump na Vercel deploy-u) → 3D/3E.
