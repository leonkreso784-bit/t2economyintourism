...
**▶ F4 (Admin CRUD) U TIJEKU (grana `foundation/f4` = preview, NE produkcija; ADR-021 + `CRUD_PLAN.md`):** ✅ F4.1 admin identitet (`profiles`+`is_admin()`+RLS, Leon seedan) · ✅ F4.2 write-path RLS + `content_versions` (undo+audit, live-dokazano) · ✅ F4.3a/b admin UI (detekcija + profil-ulaz + `#admin-page` read-only viewer; 3 buga nađena živom prijavom+popravljena). ✅ **Arhitektura predmeta ADR-022/`CATALOG_ARCHITECTURE.md`** (za HR-ekspanziju NAKON F4). **Odluka (2026-07-06): dovršiti F4 PRIJE HR-ekspanzije** (3 smjera FMTU, ~3mj runway). [[live-login-verifies-crud]]
**🆕 NASTAVAK (2026-07-08→07-11, `foundation/f4` = preview):** ✅ F4.3c edit kartice end-to-end + propagacija na final · ✅ **F4.4 quiz/fill/learn editori** · Playwright LOGIN (`test:authed`). Nastavak F4 ide kroz **UGC-stazu** (`EDITOR_PLAN.md` §12, model draft→objavi): ✅ **U1 staging Supabase** (`40dc07b`) · ✅ **U2a stabilni id-jevi na svih 18** (`b490172`, 2026-07-11). **⬜ DALJE: U2.5 (ADR-022) / U2b → U3 draft-sloj → U4 publish-RPC → …** — **živi tracker je odsad `EDITOR_PLAN.md` §12** (ovaj ROADMAP je od F4 nadalje sažetak). Tim: **+Saša** (content, paralelna S-staza — `TEAM.md`).
