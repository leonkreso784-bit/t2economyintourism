...
- **▶ STANJE 2026-06-11 (DEPLOYANO do `a6a62e2` = B3.6+B3.7 LIVE; B3.8 commitan lokalno, NEDEPLOYANO):** GOTOVO → **engine (FAZA 0–2)** + **K1 vježbe (FAZA 3 B3.1–B3.5, Ch1–6)** + **RV-1/RV-2** + **FAZA 4** (Accounting → 3 lekcije) + **B3.6 (Ch11 Depreciation, 5)** + **B3.7 (Inventory FIFO/LIFO/Average, 4)** + **B3.8 (Ch9/10 Restaurant/Hotel ratios, 4)**. Content pack `data/accounting/exercises.js` = **29 vježbi** (16 K1 Ch1–6 + 5 K2 Ch11 + 4 K2 Ch9/10 ratios + 4 K2 Inventory/„Other"). Cache `?v=20260635`.
- **▶ SLJEDEĆE — otvori §6, prva prazna cigla = `B3.9` (K2 Ch12 — Analyzing FS: `ratio` current ratio, horizontal/vertical analiza + `numeric`).** Midterm 2 „Exercises" tab sad ima **Chapter 9 / 10 / 11** + **Other** (inventory); ostatak K2 (Ch7/8/12/13/14/15-16) još nema vježbe. Plan B3.9–B3.11: Analyzing FS (Ch12), K2 journal ALE+RE (B3.10), TF/MC za Ch7/8/13/14/15-16 (B3.11). Izvori = `tmp-acc/img/` (Exercises-12/13/14/15/16, gitignored — proklikaj prije autoriranja). **Napomena:** USAR/USALI klasifikacija (Ch9-1/10-1) ostala NEAUTORIRANA jer fali službeni answer-key (riskira krivo ocjenjivanje) — vrijedi je dodati ako se nađe key. Sve nove vježbe `lesson:'second-midterm'` (ili `'final'`) + `chapter:N` (ili bez → „Other"). **Obrazac za randomizaciju:** `params` (pickParams: `{min,max,step}`/`{choices:[…]}`) + `generate(p)` vraća `{prompt,givens?,fields,solution}` spojen preko definicije → „New numbers". Drži `params` takvima da odgovori ispadnu cijeli (ratio: choices za stope/cijene; %-polja se unose kao broj npr. 35, ne 0.35). Alternativa: restruktura sljedećeg sem-1 predmeta (Entrepreneurship/E-Business) po FAZA-4 obrascu.
- Provjere: `npm run test:unit` (**95/95 exercises-core + 13/13 acc-kernel**), `npm run verify` (0/0), `npm run test:responsive` (**36/36**). Engine = `js/exercises-core.js` (graderi choice/numeric/statement/classify/journal + parse/numEq/gradeSet/pickParams) + `js/acc-kernel.js` (double-entry) + `js/exercises.js` (WIDGET registry, 6 tipova, 3 moda, randomizacija, napredak, free journal+T-konta) + `css/exercises.css`. Content pack = `data/accounting/exercises.js` (`window.accountingExercises`, **29 vježbi**: 16 K1 Ch1–6 + 5 K2 Ch11 + 4 K2 Ch9/10 ratios + 4 K2 Inventory/„Other").
