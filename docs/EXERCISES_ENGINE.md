...
- **▶ STANJE 2026-06-12 (DEPLOYANO do `d241eaf` = B3.6–B3.9 LIVE; B3.10 commitan lokalno, NEDEPLOYANO):** GOTOVO → **engine (FAZA 0–2)** + **K1 vježbe (FAZA 3 B3.1–B3.5, Ch1–6)** + **RV-1/RV-2** + **FAZA 4** (Accounting → 3 lekcije) + **B3.6 (Ch11 Depreciation, 5)** + **B3.7 (Inventory, 4)** + **B3.8 (Ch9/10 ratios, 4)** + **B3.9 (Ch12 Analyzing FS, 5)** + **B3.10 (K2 journal revenue/expense/RE, 3)**. Content pack `data/accounting/exercises.js` = **37 vježbi** (16 K1 Ch1–6 + 4 K2 Ch9/10 ratios + 5 K2 Ch11 + 5 K2 Ch12 + 7 K2 „Other": 4 Inventory + 3 journal/RE). Cache `?v=20260637`.
- **▶ SLJEDEĆE — otvori §6, prva prazna cigla = `B3.11` (TF/MC za Ch7/8/13/14/15-16 — `choice`).** Time je K2 plan KOMPLETAN. Midterm 2 „Exercises" tab sad ima **Chapter 9 / 10 / 11 / 12** + **Other** (inventory + journal/RE); fale samo konceptualni TF/MC setovi za Ch7/8 (computerised accounting), Ch13 (annual reports), Ch14 (credit/debit cards + budgeting), Ch15-16 (internal control of cash). Izvori = `tmp-acc/img/` (Exercises-13/14/15/16, gitignored — proklikaj prije autoriranja; gdje fali answer-key, koristi samo univerzalne činjenice). **Napomena:** USAR/USALI klasifikacija (Ch9-1/10-1) NEAUTORIRANA jer fali službeni answer-key. Sve nove vježbe `lesson:'second-midterm'` (ili `'final'`) + `chapter:N` (ili bez → „Other"). **Obrazac za randomizaciju:** `params` (pickParams: `{min,max,step}`/`{choices:[…]}`) + `generate(p)` vraća `{prompt,givens?,fields,solution}` → „New numbers". Drži `params` takvima da odgovori ispadnu cijeli/≤2-decimale (%-polja kao broj npr. 35). Alternativa nakon B3.11: restruktura sljedećeg sem-1 predmeta (Entrepreneurship/E-Business) po FAZA-4 obrascu.
- Provjere: `npm run test:unit` (**95/95 exercises-core + 13/13 acc-kernel**), `npm run verify` (0/0), `npm run test:responsive` (**36/36**). Engine = `js/exercises-core.js` (graderi choice/numeric/statement/classify/journal + parse/numEq/gradeSet/pickParams) + `js/acc-kernel.js` (double-entry) + `js/exercises.js` (WIDGET registry, 6 tipova, 3 moda, randomizacija, napredak, free journal+T-konta) + `css/exercises.css`. Content pack = `data/accounting/exercises.js` (`window.accountingExercises`, **37 vježbi**: 16 K1 Ch1–6 + 4 K2 Ch9/10 ratios + 5 K2 Ch11 + 5 K2 Ch12 + 7 K2 „Other" [4 Inventory + 3 journal/RE]).
