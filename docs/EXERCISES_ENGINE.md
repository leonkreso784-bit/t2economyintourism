...
- **▶ STANJE 2026-06-11 (DEPLOYANO do `d09bc7d`; B3.6 commitan lokalno, NEDEPLOYANO):** GOTOVO → **engine (FAZA 0–2)** + **K1 vježbe (FAZA 3 B3.1–B3.5, Ch1–6)** + **RV-1/RV-2** + **FAZA 4** (Accounting → 3 lekcije) + **B3.6 (prve K2 vježbe — Ch11 Depreciation, 5 vježbi)**. Content pack `data/accounting/exercises.js` = **21 vježbe** (16 K1 Ch1–6 + 5 K2 Ch11). Cache `?v=20260633`.
- **▶ SLJEDEĆE — otvori §6, prva prazna cigla = `B3.7` (K2 Inventory FIFO/LIFO/Average COGS).** Midterm 2 „Exercises" tab sad ima **Ch11** (više nije prazan); ostatak K2 (Ch7/8/9/10/12/13/14/15-16) još nema vježbe. Plan B3.7–B3.11: inventory FIFO/LIFO, Restaurant/Hotel ratios (Ch9/10), Analyzing FS (Ch12), K2 journal ALE+RE, te TF/MC za Ch7/8/13/14/15-16. Izvori = `tmp-acc/img/` (Exercises-9..16, gitignored). Sve nove vježbe `lesson:'second-midterm'` (ili `'final'`) + `chapter:N`. **Obrazac za randomizaciju:** `params` (pickParams spec: `{min,max,step}`/`{choices:[…]}`) + `generate(p)` vraća `{prompt,fields,solution}` spojen preko definicije → „New numbers". Drži `params` takvima da odgovori ispadnu cijeli (npr. life∈{4,5,10}). Alternativa: restruktura sljedećeg sem-1 predmeta (Entrepreneurship/E-Business) po FAZA-4 obrascu.
- Provjere: `npm run test:unit` (**95/95 exercises-core + 13/13 acc-kernel**), `npm run verify` (0/0), `npm run test:responsive` (**36/36**). Engine = `js/exercises-core.js` (graderi choice/numeric/statement/classify/journal + parse/numEq/gradeSet/pickParams) + `js/acc-kernel.js` (double-entry) + `js/exercises.js` (WIDGET registry, 6 tipova, 3 moda, randomizacija, napredak, free journal+T-konta) + `css/exercises.css`. Content pack = `data/accounting/exercises.js` (`window.accountingExercises`, **21 vježbi**: 16 K1 Ch1–6 + 5 K2 Ch11).
