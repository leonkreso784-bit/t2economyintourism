...
- **▶ STANJE 2026-06-11 (DEPLOYANO do `d09bc7d`; B3.6+B3.7 commitani lokalno, NEDEPLOYANO):** GOTOVO → **engine (FAZA 0–2)** + **K1 vježbe (FAZA 3 B3.1–B3.5, Ch1–6)** + **RV-1/RV-2** + **FAZA 4** (Accounting → 3 lekcije) + **B3.6 (Ch11 Depreciation, 5)** + **B3.7 (Inventory FIFO/LIFO/Average, 4)**. Content pack `data/accounting/exercises.js` = **25 vježbi** (16 K1 Ch1–6 + 5 K2 Ch11 + 4 K2 Inventory/„Other"). Cache `?v=20260634`.
- **▶ SLJEDEĆE — otvori §6, prva prazna cigla = `B3.8` (K2 Restaurant/Hotel ratios, Ch9/10 — `ratio`: average check, seat turnover, food/labor cost %, RevPAR…).** Midterm 2 „Exercises" tab sad ima **Chapter 11** + **Other** (inventory); ostatak K2 (Ch7/8/9/10/12/13/14/15-16) još nema vježbe. Plan B3.8–B3.11: Restaurant/Hotel ratios (Ch9/10), Analyzing FS (Ch12), K2 journal ALE+RE, te TF/MC za Ch7/8/13/14/15-16. Izvori = `tmp-acc/img/` (Exercises-9/10/12/13/14/15/16, gitignored — proklikaj prije autoriranja). Sve nove vježbe `lesson:'second-midterm'` (ili `'final'`) + `chapter:N` (ili bez chaptera → „Other" za topic koji nije numerirano poglavlje). **Obrazac za randomizaciju:** `params` (pickParams spec: `{min,max,step}`/`{choices:[…]}`) + `generate(p)` vraća `{prompt,fields,solution}` spojen preko definicije → „New numbers". Drži `params` takvima da odgovori ispadnu cijeli (life∈{4,5,10}; inventory: bira jedinice/cijene da prosjek/slojevi daju cijele brojeve). Alternativa: restruktura sljedećeg sem-1 predmeta (Entrepreneurship/E-Business) po FAZA-4 obrascu.
- Provjere: `npm run test:unit` (**95/95 exercises-core + 13/13 acc-kernel**), `npm run verify` (0/0), `npm run test:responsive` (**36/36**). Engine = `js/exercises-core.js` (graderi choice/numeric/statement/classify/journal + parse/numEq/gradeSet/pickParams) + `js/acc-kernel.js` (double-entry) + `js/exercises.js` (WIDGET registry, 6 tipova, 3 moda, randomizacija, napredak, free journal+T-konta) + `css/exercises.css`. Content pack = `data/accounting/exercises.js` (`window.accountingExercises`, **25 vježbi**: 16 K1 Ch1–6 + 5 K2 Ch11 + 4 K2 Inventory/„Other").
