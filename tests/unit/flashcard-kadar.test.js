...
tvrdi(/function okretKartice\(zelim\)/.test(FC_JS) && /classList\.add\('is-turning'\)/.test(FC_JS)
    && /addEventListener\('transitionend', naKraj\)/.test(FC_JS) && /setTimeout\(kraj, OKRET_MS\)/.test(FC_JS),
    '`okretKartice()`: `is-turning` prije promjene, skida se na `transitionend` ILI rezervni timer');
tvrdi(!/classList\.toggle\('flipped'\)/.test(FC_JS),
    'nitko više ne togglea `flipped` izravno — klik, dodir i tipka idu kroz `okretKartice()`');
tvrdi((FC_JS.match(/okretKartice\(\);/g) || []).length >= 2,
    '`flipCard` (klik/tipka) i dodir bez pomaka (`swipeUp`) zovu ISTI `okretKartice()`');
