...
test.describe('F1/9 — kartice kao Tinder-špil na dodiru', () => {
    ...
    test('desno = znam, lijevo = ne znam — upis kroz markKnown/markUnknown, brojke na ekranu', async ({ page }) => {
        ...
        // BUG-047 (12.09.): zapis je po IDENTITETU kartice (string), ne po poziciji — `includes(0)` je do
        // spajanja 13.09. TVRDIO kvar (isti obrat kao app-state.spec ④); `cardIdentity` je globalna funkcija.
        const zapis = await page.evaluate(() => ({
            learned: progress.flashcardsLearned,
            id0: cardIdentity(AppState.cards.deck[0])
        }));
        expect(zapis.learned).toContain(zapis.id0);
