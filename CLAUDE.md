...
## Stanje — TRENUTNO (2026-08-06; povijest: `docs/records/HISTORY.md` · `docs/records/PROGRESS.md` · `docs/archive/EDITOR_PLAN.md §12`)
- **🎯 SLJEDEĆI RAD: FRONTEND REDIZAJN.** Osobni UGC-graditelj je **ISPORUČEN NA PRODUKCIJU** (F5, 2026-08-06) → Leonov redoslijed *„sve mora savršeno raditi prije nego ga uredimo"* je time otključao vizualnu fazu. Iza redizajna: **objava/dijeljenje + MCP.**
- **⏳ DVIJE GRANE ČEKAJU MERGE (2026-08-07, oboje zeleno, `main` netaknut):** **`fix/studio-katex`** (`39e5d09`, **pushana**) = BUG-021, formule su u Studiju bile sirovi LaTeX — `studio.js` nikad nije zvao `renderMath()` nakon `innerHTML`; za osobno gradivo je to značilo **nikad tipografirano** (čvor se gleda samo u Studiju). **`feature/inline-math`** (grana od prethodne) = **matematika USRED rečenice** — run dobiva `math:true`, gumb **√x** u plutajućoj traci; renderer i dalje samo `esc()`-a i emitira `\(tex\)` kao tekst, tipografira `renderMath` na prikazu. Gate: unit 77/0+29/0 · `test:authed` **55/55** · preflight 0. **⚠️ Ostaje otvoreno: MathLive keypad** — Leon ga je ocijenio lošim, ali svjesno NIJE diran u ovoj cigli.
...
## Ključne odluke (jedan red po ADR-u; puni tekst `docs/records/DECISIONS.md`)
