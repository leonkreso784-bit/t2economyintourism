...
- **▶ FAZA 3 (performanse) — 3C.1+3B+3A ✅ DEPLOYANO NA PRODUKCIJU (2026-07-05, main `c115a5d..868dc9f`; live-verified: token `20260705140655`, `/sw.js` `max-age=0,must-revalidate` + `SW_VERSION`+`res.ok`+`sw:skipWaiting`, bundle immutable, sw-register update-flow servira):** redoslijed = najsigurnija cigla prva → **3C → 3B → 3A → 3D → 3E** (SW zadnja, najrizičnija).
  ...
  **✅ 3E.2 moderate landmarks (OPUS, ista grana):** SVE 4 stranice sad **100% axe-clean** (0 violationa bilo kojeg levela). `region`: `.hero-stats`/`.landing-cta` labelirani + **nested `<header>`/`<footer>` u `<section>` gube implicitni banner/contentinfo → eksplicitni `role="banner"`/`role="contentinfo"`** (landing-nav/footer + study/browse/profile zaglavlja; jedna stranica vidljiva → bez duplikata). `heading-order`: footer h4→h3. Atribut-only osim footer tag+CSS. Gate: **Playwright 185/0**, axe 4/4 clean.
  **⬜ DALJE: deploy F3-ostatak (3D+3E, uz potvrdu)** → opcionalno **3C.2** (auto-bump na Vercel deploy-u), geo-JPG→WebP, FA async, PWA icon-512. **🧭 Nakon F3: F4 CRUD → F5 SRS → F6 sigurnost → UGC → tek onda nazad na sadržaj (ADR-018).** [[foundation-pivot]]
