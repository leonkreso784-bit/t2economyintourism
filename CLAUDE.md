...
- **`npm run preflight`** — svi brzi deploy-gate-ovi u jednom (**check:lockfile** · verify · bump:check · css-drift · **check:tailwind** · **check:cdn** · **check:palette** · **check:safearea** · **check:budget** · **check:seo** · **check:contrast** · typecheck · schema · export-drift · check:docs · **check:state** · unit); pokreni PRIJE svakog main-pusha (i pre-push hook ga automatski vrti na main).
...
- **`npm run check:budget`** (T6) — **koliko koda dobije posjetitelj koji SAMO otvori stranicu.** Dvije provjere, i prva postoji zato što je druga brojka: ① **nijedna editorska datoteka nije na posjetiteljevu putu** (tvrdnja o SASTAVU) · ② ukupni prijenos skripti **≤ 200 KB** (tvrdnja o TEŽINI). Bez ① bi se budžet dao ispuniti i minifikacijom editora, a cigla nije bila „smanji bajtove" nego *„alat koji posjetitelj nikad ne otvori ne smije mu ni stići"*. ⚠️ **Mjeri PRENESENE bajtove, ne veličinu na disku** — budžet dolazi iz Lighthousea; zapisano „3,7× preko" bilo je računato sirovo, a stvarno je bilo 1,17×. Sirovo se i dalje **ispisuje** (mjeri parsiranje), ali ne odlučuje. Lokalno i brzo → **u preflightu**.
- **`npm run check:seo`** — **ono što tražilica i pretpregled linka VIDE.** Povod nije bila želja za prometom nego **netočnost**: `meta description` je platformu opisivao kao *„exam prep for FMTU Opatija — Hospitality Management"*, i isto je stajalo u `og:` i `twitter:` — dakle cijela stranica je tražilicama govorila da je skripta za jedan smjer jednog fakulteta, dok ADR-029 kaže da je UGC glavni proizvod. **Tri opisa su uz to bila međusobno RAZLIČITA**, pa dvije od sedam provjera mjere baš to: `og:title` == `<title>` i `twitter:description` == `description`. *Ako je jedna priča, mora biti jedan tekst.* Ostalo: sitemap se poklapa s **diskom** (popis se ne piše rukom — T6 je to pravilo platio dvaput) · `robots.txt` imenuje sitemap i **ne smije `Disallow`-ati stranicu s `noindex`om** (zabrana obilaska spriječi čitanje noindexa → stranica ostane u indeksu) · duljine naslova/opisa · `og:image` **postoji i JEST 1200×630** · JSON-LD **PARSIRA** (tražilica neispravan blok tiho odbaci, pa bi „ima ga" prolazilo i nad pokvarenim). `--write` regenerira sitemap. Bez mreže i preglednika → **u preflightu**.
- **`npm run build:og`** — crta `og-cover.png` **1200×630** (bila je `icon-512.png`, kvadratna aplikacijska ikona → svaki podijeljen link je davao kvadratić umjesto kartice). Boje čita iz `css/tokens.static.css`, tekst iz `js/i18n.js` — ručno nacrtana slika bila bi **četvrta kopija palete i peto mjesto s tekstom**. ⚠️ PNG je **zamrznut u trenutku generiranja** (font se ne ugrađuje: Windows → Segoe UI, macOS → SF), pa se **commita** — posjetitelj vidi bajtove iz repozitorija, ne ovu skriptu. Traži preglednik → **NIJE u preflightu**; dimenzije mjeri `check:seo` nad datotekom. *Gate mjeri artefakt, ne alat koji ga radi.*
...
- **⚠️ ZELENO LOKALNO NIJE ZELENO (2026-08-24).** Windows i Linux ne crtaju isti font istom
  širinom (~4 px), i to je dovoljno da **brana promijeni ishod bez ijedne promjene u proizvodu**.
  U jednom pushu je to naplaćeno tri puta: marker landinga se lomio preko dva retka (T5 je
  `nowrap` odbacio **bez mjerenja**; fraza troši 58 % stupca, prelijevanja nema do 1,9×),
  phone-osnovica je **poznat nalaz brojala kao nov** jer joj je izmjerena brojka bila u imenu,
  a landing je na 320 px prolazio sa **zalihom od 21 px = 3,7 % ekrana**. *Tvrdnja koja prolazi
  s 3,7 % rezerve ne mjeri ispravnost nego sreću.* Otud: **CI je jedini sudac za mjere ovisne o
  crtaču**, a nova tvrdnja o rasporedu treba **sondu sa širim slovima** prije nego se proglasi
  gotovom. ⚠️ **Padovi u CI-ju se čitaju kao ANOTACIJE** (`github` reporter, uključen samo uz
  `process.env.CI`) — inače se ime palog testa dobiva samo iz artefakta od 87 MB koji traži
  prijavu, pa svaki pokušaj košta rundu od ~18 min. ⚠️ **GitHubov javni API ima 60 zahtjeva/h** —
  ne provjeravati CI u petlji, inače ostaneš bez očitanja baš kad ti treba.
...
- **`npm run test:responsive`** — Playwright (iPhone profili, default suite). ⚠️ **Prijava zna pasti iz utrke, i to NIJE kod — ali ima DVA lica.** ① `JWT issued at future`: lokalni sat točan (<1 s), isti token kroz direktan HTTP prošao (`is_admin` = true, 200); sub-sekundna utrka između sata koji `iat` **izdaje** (GoTrue) i onoga koji ga **provjerava** (PostgREST). ② **`is_admin() = false` uz PRAZAN `rpcError`** (imenovano tek 2026-08-24, dotad se krivo pripisivalo prvom licu): poziv **uspije**, ali je kontekst još ANONIMAN, pa funkcija uredno vrati `false`. Drugo lice je opasnije jer **nema greške** — a `auth.setup` je preduvjet cijelog `authenticated` projekta, pa jedan promašaj obori **92 testa koja nikad ne krenu** (dogodilo se 2× u 7 prolaza). Od tada `auth.setup` **čeka stanje** (6 × 250 ms) i ispisuje broj pokušaja, pa se utrka razlikuje od stvarne uloge. Ne tražiti uzrok u vlastitom kodu. · **`npm run test:authed`** — pozitivan admin-put (storageState; traži `TEST_ADMIN_EMAIL/PASSWORD` u `.env`; CI = zaseban secret-gated job).
...
## Stanje — TRENUTNO (2026-08-24)

> **✅ CRVENI ALARM JE UGAŠEN — FAZA TELEFON (T0–T6) I BUG-032 SU NA PRODUKCIJI**
> (2026-08-24, `2e9fff9..82f8560`, 45 commita; verifikacija posluženim sadržajem = zadnji **🚀**
> redak u `CHANGELOG.md`). Povod je bio Leonov nalaz na iPhoneu 16: *„cijeli frontend na
...
>
> **✅ PREDSTAVLJANJE + SEO-TEMELJI GOTOVI** (2026-08-24, spec **§9.15**, ista grana, **nije
> deployano**). Leonova odluka o smjeru: **B — oboje ravnopravno**; javni opis **ostaje
> engleski** (izričito, iako je publika hrvatska). ⚠️ **Povod je bio nalaz, ne želja za
> prometom:** ista FMTU-only rečenica koju je `about` izgubio stajala je i u `meta
> description`, `og:` i `twitter:` — a **sva tri opisa bila su međusobno različita**. Isporučeno:
> jedan tekst na četiri mjesta · **`og-cover.png` 1200×630** (bila kvadratna ikona → svaki
> podijeljen link je davao kvadratić) · `robots.txt` · `sitemap.xml` **generiran s diska** ·
> minimalan JSON-LD · `meta keywords` obrisan · nova brana **`check:seo`**.
> ⚠️ **Što NIJE napravljeno i ne planira se prešutno:** prave adrese umjesto hash-ruta
> (indeksabilno je **5 stranica**, 24 predmeta žive iza `#/`), `Course` shema i **SEM**. Prvo je
> arhitektonska odluka, a dio te rasprave je **već presuđen** — ADR-028: doseg dijeljenja je
> **link s tajnim tokenom, bez javne biblioteke**, dakle korisničko gradivo **po dizajnu nije
> javno pronađljivo**, pa SEO doseže samo katalog i marketinške stranice.
>
...
>
> **🔒 TVRDE ODLUKE O DEPLOYU.** ① *„Ništa ne ide na produkciju dok cijeli frontend ne bude
> riješen"* (Leon, 2026-08-19) — **Leon je tu odluku sam promijenio 2026-08-24** i odobrio
> merge (*„moze merge na main"*) iako C4–C7 i POLICA nisu gotovi. Obrazloženje koje je stajalo
> iza odluke prestalo je vrijediti: prepreka je bila *„seoba/rad bi zamrznuo jedino što
> korisnicima smeta"*, a to je bio telefon — koji je time riješen. **Odluka o odgodi je time
> POTROŠENA, ne ukinuta:** sljedeći deploy opet traži izričit OK. ② **broj commita izvan
> produkcije NIJE nalaz i NE SPOMINJE SE** (*„ZNAM KADA ZELIM PUSTIT NESTO NA PRODUKCIJU"*) —
> **ova stoji netaknuta.** Pravilo #2 time dobiva dopunu: ne samo da se ne smije pushati bez
> OK-a, nego se na to ne smije ni **nagovarati**. [[leon-decides-deploys]]
>
...
> ⚠️ **Poznato TADA, pri T0** (ovdje je do 2026-08-22 pisalo „poznato danas", a čitalo se kao
> trenutno stanje — točno razred greške zbog kojeg postoji `check:state`): ② **25 ekrana**
> (320 px: browse **49 %**) · ④ **15** (na 320 px kromo+banner = **84–89 %**) · ⑤ **5**
> (`span.crumb` odrezan na 30 %) · prijavljeno **4**; ① i ③ su i tada bili **0**.
> **DANAS: OSNOVICA JE PRAZNA — brana traži NULU.** Put dotle: 8 nalaza → 4 (BUG-032 pojeo
> četiri na `lessons`) → **0** (cigla `about`, §9.14, pojela preostala četiri). Trenutnu brojku
> uvijek zna `tests/phone-baseline.json`, ne ova proza.
...
> ⚠️ **Sedam padova koje je cigla usput napravila bilo je u TESTNOJ INSTALACIJI, ne u proizvodu**,
> i uzrok je bio jedan: otkad editor ima vlastiti dokument, spec koji **i stvara materijal i vozi
> editor** treba DVIJE stranice (`SokratMaterials` samo u aplikaciji, `studioBridge` samo na
> `editor.html`). Popravak **nije išao po testovima nego po HELPERIMA**: `publishSections` sam
> otvara editor, `rmNode` se sam vraća u aplikaciju — pozivatelj ne pamti nijedno. *Kad se
> proizvod prelomi na dva dokumenta, znanje o tome gdje koja polovica živi ide u helper, inače
> ga prepisuje svaki spec.*
> ⚠️ **Jedan pad je SKRIVAO drugi:** `f4-e2e` je u prvom prolazu srušio `auth.setup`
> (`is_admin() = false`, viđeno **1× u 5 prolaza**, i to NIJE zapisani `JWT issued at future`),
> pa je njegov pravi kvar (`setNode` na krivoj stranici) izašao tek kad je auth prošao. Zbog toga
> je zapisani uzrok za taj spec bio kriv. *Pad koji sruši pripremu sakrije sve iza sebe — popis
> padova nije popis uzroka.*
> ⚠️ **Zamka s alatom:** `npm test … | tail -30` javlja izlazni kod **cijevi**, pa je pozadinska
> naredba prijavila „exited with code 0" uz sedam padova. Čitaj popis padova, ne izlazni kod.
>
> **Što je T6 (izmjereno) donio:** Leon je od tri ponuđena reza izabrao najveći: editor ne dobiva
> lijeno učitavanje nego **vlastitu stranicu** (`editor.html`). Posjetiteljev put **mrežom
> 234 → 164 KiB** (ispod budžeta od 200), **sirovo 755 → 519**, **41 → 36 skripti**, editorskih
> datoteka na putu **7 → 0**.
> ⚠️ **„3,7× preko budžeta" bilo je u KRIVOJ JEDINICI:** brojka je računata na sirovim bajtovima,
> a budžet dolazi iz Lighthousea, koji mjeri **prenesene** — u ispravnoj jedinici zatečeno je bilo
> **1,17×**, pa je izlazak editora sam po sebi dovoljan. *Brojka može biti točna i svejedno
> savjetovati krivo ako je u krivoj jedinici* (isti razred kao `palette:breakdown`).
> ⚠️ **Rez nije išao po datoteci nego KROZ nju:** `admin.js` je nosio i „jesi li ti admin", što
> aplikacija treba i bez editora → **`js/admin-reveal.js` ostaje, `admin.js` seli**; **`node-images.js`
> OSTAJE** jer ga traži `blocks-renderer.js` (studentov put), a `initTheme()` je izašao u
> **`js/theme.js`** (init.js nije „boot" nego boot APLIKACIJE).
> ⚠️ **Stranica rješava ono što je K1 namjerno izbjegao** („deep-link bi pokazao prazan editor
> bilo kome tko zna adresu"): čuvar ne pokazuje ništa dok identitet nije razriješen, a
> **vlasništvo se NE čita iz adrese** — `?node=` nosi samo ID, ime dolazi iz baze kroz RLS, pa je
> jedan upit i identitet i provjera.
> 🐞 **Tri kvara našla je TVRDNJA, ne čitanje koda:** `navigateTo` nije bila navigacija nego
> **spoj** („nacrtaj" pa „pokaži") i prijevod je prenio pola poziva · gumb „natrag" bio je vezan
> unutar `poruka()`, dakle **samo kad čuvar odbije** (sidro s dva pogotka, skripta bez provjere
> jedinstvenosti) · rani `return` u mjeraču preskočio je `smiriPrikaz()` i dao lažni nalaz.
> ⚠️ **Dva gatea nisu vidjela novu stranicu** (v. `check:cdn` / `check:tailwind` gore) — popravak
> **briše ručni popis** umjesto da mu doda ime. **Nova brana: `npm run check:budget`.**
> ⚠️ **Popis „tko dira editor" bio je nepotpun:** dvije datoteke stranicu ne spominju, ali
> **čekaju njezine globale** → test ne padne nego visi. *Ovisnost nije samo „tko spominje" nego i
...
- **NA PRODU** (**točan SHA, token i Vercel-ref = zadnji „🚀" redak u `docs/records/CHANGELOG.md`** — ovdje se namjerno ne prepisuju, mijenjaju se svakim deployem; ADR-027): **24 predmeta** (17 EN u Supabase + **7 HR file-served**) · auth + cloud-sync + profil + GDPR · **Studio editor** (admin) · **osobni UGC-graditelj „Moji materijali"** · **faza „Materijal od nule do učenja"** — materijal se gradi od nule, iz njega se uči, boje sekcije vidljive u sva 4 moda · rizik-sprint 7/7 · GA4 + Sentry (consent-gated) · SW offline · i18n HR/EN · CI + deploy-guard · **(od 2026-08-24) faza TELEFON T0–T6** — telefon je mjerena površina, editor ima vlastitu stranicu (`editor.html`) i sišao je s posjetiteljeva puta · **BUG-032** — katalog je dostupan tipkovnicom i čitačem ekrana · **KOSTUR K1–K4a** — devet stranica ima devet **dijeljivih adresa**, jedan model vraćanja, jedna gornja traka · **landing C+D** (katalog s tražilicom, vlastito gradivo, četiri načina, MCP „uskoro"). Tablica predmeta: `docs/subjects/README.md`.
- **❓ OTVORENO — RAZGOVARANO 2026-08-24, ALI NIJE PRESUĐENO. Ne planirati kao dogovoreno.**
  Leon je postavio tri pitanja i na moje preporuke **nije odgovorio** — pitanja su ostala
  otvorena jer je razgovor otišao na CI. Sve brojke ispod su izmjerene tog dana:
  - **Birač tema na landingu.** Mehanika je GOTOVA: 4 teme (`academic`/`paper`/`chalk`/`mint`),
    `setTheme` pamti izbor, `check:contrast` vrti 164 provjere kroz sve 4, a **klik-vezanje već
    stoji u kodu** (`js/init.js`, `.theme-toggle` + `data-set-theme`) uz komentar da gumb
    namjerno nije u markupu. Blokira ga **24 pravila** (`palette:breakdown` → „FATALNO"), NE
    C4–C7: ostalih 89 je blijedo ili neusklađeno, ali čitljivo. ⚠️ Peta kontrola u traci
    landinga nije besplatna (K3/BUG-029) — vjerojatnije mjesto je red kvadratića u heroju.
  - **OAuth (Google/Apple).** Brojka koja to opravdava: **5 registriranih korisnika**, od toga
    **3 Leonova vlastita** (mjereno na produkciji 2026-08-24), jer je e-mail+lozinka jedini put.
    Google je besplatan i ne čeka redizajn; **Apple traži ~99 $/god** i nema smisla bez iOS
    aplikacije; „Sign in with ChatGPT" je **NEPOTVRĐEN** za aplikacije — ne obećavati.
    A0 (dijalog) ide **zajedno** s A1, inače se prepravlja dvaput.
  - **Self-host Supabase prije OAuth-a.** Leon: *„mislio sam prije toga prebacit subabase na
    lokalno i rentat neki vps."* Argument koji to podupire: sve tri čekajuće auth-stavke
    (Google, lozinka 6→8, zaštita od procurjelih lozinki) su **Supabase-konfiguracija**, pa bi
    se poslije seobe radile dvaput — a seoba mijenja i sam URL, dakle i redirect URI.
    ⚠️ **Backlog NE spominje staging:** danas write-testovi gađaju zaseban HOSTANI projekt
    (`czljmvigkgiajzjxtndq`), pa bi seobom trebale **dvije instance**, ne jedna.
    ⚠️ **NEODGOVORENO PITANJE koje Leonu dugujem:** može li staging biti na drugom laptopu, a
    produkcija na rentanom VPS-u. Kratko: da za razvoj, ali CI job ne može do laptopa iza
    kućnog rutera bez tunela, a brana koja ovisi o tome je li laptop upaljen prestaje biti brana.
  - **Migracija računa je jeftinija nego što je zapisano:** od 5 računa su 3 Leonova, dakle
    stvarnih vanjskih korisnika je **dvoje**. Nepovratan je samo sadržaj **dva Storage bucketa**
