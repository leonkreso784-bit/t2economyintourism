...
- **▶ FAZA 3 (performanse) — 3C.1+3B+3A ✅ DEPLOYANO NA PRODUKCIJU (2026-07-05, main `c115a5d..868dc9f`; live-verified: token `20260705140655`, `/sw.js` `max-age=0,must-revalidate` + `SW_VERSION`+`res.ok`+`sw:skipWaiting`, bundle immutable, sw-register update-flow servira):** redoslijed = najsigurnija cigla prva → **3C → 3B → 3A → 3D → 3E** (SW zadnja, najrizičnija).
  ...
  **✅ 3D.1 optimizacija slika (OPUS, grana `foundation/f3d`; NIJE deployano):** `blind-map.png` (**1.52 MB**, jedina velika slika) → **`blind-map.webp` q85 = 39 KB (−98%, 40×)**, vizualno identično (karta se crta na canvas → format transparentan). `js/blind-map.js`: WebP→PNG fallback (PNG ostaje za ~1.5% preglednika) + dodan `?v=` token (prije izostavljen). `static-server.js` `.webp` MIME. Novi `tests/blind-map.spec.js`. Nalaz: `loading="lazy"` VEĆ na svim learn slikama; blind-map ≈95% težine slika → 3D.1 = glavni dobitak. Cache `20260705161843`. Gate: **Playwright 185/0**, verify/typecheck/unit/bump:check/build:css-check 0.
  **⬜ DALJE (opcionalno, diminishing returns): ** 1 inline geo-slika lazy + geo-JPG→WebP + PWA icon-512 → **3E** (a11y) → **3C.2** (auto-bump na Vercel deploy-u) → deploy F3-ostatak. **🧭 Nakon F3: F4 CRUD → F5 SRS → F6 sigurnost → UGC → tek onda nazad na sadržaj (ADR-018).** [[foundation-pivot]]
