...
- **📊 FAZA: 4½ od 5 kriterija** (grana `docs/stage-a`, `main` NETAKNUT, sve zeleno):
  **M1 ✅** svi modovi u praznom materijalu (`presentModes` je crtao mod samo za NEPRAZAN niz → prva kartica se nije mogla dodati nikad; uređivači su radili, bili su nedostupni) · **M2 ✅** učenje iz vlastitog materijala (materijal se registrira kao **sintetički predmet** `node:<uuid>` u `subjectDataMap` → napredak/analitika/profil/sync rade bez izmjene; dirane 2 funkcije u `navigation.js`) · **M3a ✅** boja **bloka** = akcent `#rrggbb` (`lb-tint` + `--lb-acc`, emitira se samo nakon `^#[0-9a-fA-F]{6}$`; nasljeđivanje je besplatno kroz CSS-kaskadu) · **M4 ✅** sučelje prestalo obećavati (AI-gumb → „Tvoj AI / USKORO") · **M3b ⬜ JEDINO PREOSTALO** = boja kartice/pitanja/dopune (tri zasebna prikazivača `flashcards/quiz/fill` + Studio-paneli — druga površina).
- **⚠️ NIŠTA IZ FAZE NIJE VERIFICIRANO LEONOVOM ŽIVOM PRIJAVOM** — gate-ovi su zeleni protiv **staginga**, sve stoji na grani. To je točno ona razlika zbog koje je faza jednom krivo proglašena gotovom.
- **⏳ GRANE IZVAN `main`-a:** `docs/stage-a` (cijela faza + reorganizacija dokumentacije) · `docs/reorg` · Sašine `content/entrepreneurship-hr` (3) i `content/ebusiness-hr` (1) — **obje diraju `data/catalog.js` i cache-tokene → druga po redu će konfliktirati** (rebase + `npm run bump`).
