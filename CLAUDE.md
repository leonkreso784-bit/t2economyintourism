...
- **▶ FAZA 3 (performanse) — 3C.1+3B+3A ✅ DEPLOYANO NA PRODUKCIJU (2026-07-05, main `c115a5d..868dc9f`; live-verified: token `20260705140655`, `/sw.js` `max-age=0,must-revalidate` + `SW_VERSION`+`res.ok`+`sw:skipWaiting`, bundle immutable, sw-register update-flow servira):** redoslijed = najsigurnija cigla prva → **3C → 3B → 3A → 3D → 3E** (SW zadnja, najrizičnija).
  ...
  **✅ 3D+3E ✅ DEPLOYANO NA PRODUKCIJU 2026-07-06 (ff-merge `e39eb1d..b19a641` grana `foundation/f3d`→main uz izričito odobrenje; CI zelen oba job-a; Vercel preview vizualno potvrđen; live-verified: token `20260706003609`, `blind-map.webp` HTTP 200 40KB `image/webp`, `--danger-text:#f87171` u live bundle-u, `media="print"` async u index, `/sw.js` `max-age=0`). → time F3 (performanse) KOMPLETNA (3C.1+3B+3A+3D+3E svi LIVE).**
  **✅ 3D.1 optimizacija slika (OPUS):** `blind-map.png` (**1.52 MB**, jedina velika slika) → **`blind-map.webp` q85 = 39 KB (−98%, 40×)**, vizualno identično (karta se crta na canvas → format transparentan). `js/blind-map.js`: WebP→PNG fallback (PNG ostaje za ~1.5% preglednika) + dodan `?v=` token (prije izostavljen). `static-server.js` `.webp` MIME. Novi `tests/blind-map.spec.js`. Nalaz: `loading="lazy"` VEĆ na svim learn slikama; blind-map ≈95% težine slika (ali se učita SAMO u Geography → ne dira landing perf). Cache `20260705161843`.
  **✅ 3D.2 render-blocking eliminacija na landingu (OPUS):** pravi landing perf-bottleneck = 3 eksterna render-blocking CSS-a. **KaTeX CSS** (na landingu neiskorišten; komentar LAŽNO tvrdio „ne blokira" — samo JS bio `defer`) + **Google Fonts** → **ASINKRONO** (`media=print`→`onload media=all`) + `<noscript>` fallback; **Font Awesome OSTAJE render-blocking** (async=bljesak ikona; zaseban zahvat) + `preconnect` cdnjs. HTML-only → **nema bumpa**. Vizualno (screenshot) + `katex.spec` 4/4. Gate: **Playwright 185/0**. CSP-napomena F6: inline `onload`→nonce/JS-flip.
  **✅ 3E.1 a11y hardening (OPUS):** dubinski axe audit otkrio **rupu u gate-u** — postojeći a11y gate skenirao samo landing/browse/learn/profile → **flashcards/quiz/fill/progress IZVAN gate-a**, pa su kroz njih na produkciju prošli **critical** (button-name flashcard prev/next; select-name quiz selecti). Popravljeno + gate PROŠIREN (skenira sve study sekcije): `data-i18n-aria` (aria-label za ikone-gumbe) + `<label for>` quiz + **`--danger-text`** token + `--primary`→`--primary-dark` (check/th/filter) + learn h3 →`--primary-light` + box-naslovi svijetli+obojana ikona + `enhanceLearnTables()` (tabindex za skrolabilne tablice). **0 serious/critical.** Gate: **Playwright 185/0**, a11y 4/4. Cache `20260705215529`.
  **✅ 3E.2 moderate landmarks (OPUS):** SVE 4 stranice sad **100% axe-clean** (0 violationa bilo kojeg levela). `region`: `.hero-stats`/`.landing-cta` labelirani + **nested `<header>`/`<footer>` u `<section>` gube implicitni banner/contentinfo → eksplicitni `role="banner"`/`role="contentinfo"`** (landing-nav/footer + study/browse/profile zaglavlja; jedna stranica vidljiva → bez duplikata). `heading-order`: footer h4→h3. Atribut-only osim footer tag+CSS. Gate: **Playwright 185/0**, axe 4/4 clean.
  **⬜ DALJE (F3 gotova): F4 CRUD → F5 SRS → F6 sigurnost → UGC → tek onda nazad na sadržaj (ADR-018).** Opcionalni F3 follow-up (ne blokira, diminishing returns): **3C.2** (auto-bump na Vercel deploy-u), geo-JPG→WebP, FA async, PWA icon-512. [[foundation-pivot]]
- **▶ FAZA 4 (Admin CRUD) — U TIJEKU na grani `foundation/f4` (plan `docs/CRUD_PLAN.md`, ADR-021; NIJE deployano, sve lokalno):** uređivanje sadržaja kroz sučelje, bez deploya, **custom (ne CMS)**. Odluke: **direktni preglednik→Supabase RLS-write · `profiles.role` admin · grubi blob · stupnjeviti flip · safety-net (verzioniranje) od prve cigle.**
  **✅ F4.1 admin identitet (`5ee749e`):** `supabase/f4-admin.sql` — `profiles(user_id,role)` + auto-provision trigger + **`is_admin()`** (reusable RLS helper) + select-own RLS (role immutable iz klijenta). Primijenjeno na bazu (MCP) + **Leon seedan `role='admin'`** (3 ostala `user`) + `rls-check` proširen, zelen (anon vidi 0 profiles).
  **✅ F4.2 write-path + verzioniranje (`5242e52`):** `supabase/f4-content-write.sql` — admin-only insert/update/delete RLS na `subject_content` + **`content_versions`** (append-only snapshot na svaki write = **undo + audit tko/kad**, BEFORE UPDATE/DELETE trigger, SECURITY DEFINER). **Live-dokazano rollback-transakcijama (produkcija netaknuta, 51 red):** admin piše + verzija/audit zapisani; običan korisnik I anon → 0 redova. `rls-check` = 4 invarijanta.
  **✅ F4.3a/b admin UI — LIVE-VERIFICIRANO (2026-07-06, prava admin-prijava; commiti `fc655a8`+`28984fe`+`45489f7`+`0bc5e41`):** `js/admin.js` → `SokratAdmin.isAdmin()` (Supabase RPC `is_admin()`, kešir, auth-onChange) + `.admin-only` reveal + **admin kartica u profilu** („Uredi sadržaj") + **`#admin-page`** viewer (predmet→lekcija→**read-only lista kartica** kroz `SokratContent`). **3 buga nađena živom prijavom + popravljena:** `window.SokratAuth`→golo `SokratAuth` (nije window prop!) · `.admin-page` `display:none` default (curio „Admin" na dno) · native select popup `color-scheme:dark`. **POUKA: Playwright NIJE uhvatio detekcijski bug (testirao samo `isAdmin===false`) → prava admin-prijava nužna za CRUD** [[live-login-verifies-crud]]. Grana `foundation/f4` pushana = **preview** (`studymaster-git-foundation-f4…vercel.app`), NE produkcija.
  **✅ F4.3c (prva UI cigla — edit kartice end-to-end) KOMPLETNA + ŽIVO VERIFICIRANA (2026-07-08, `7d1368a`+`f208eef`):** **c-1** = kartica→„uredi"→`<sokrat-modal>` forma→write JEDNOG reda (`catalog.resolve[lessonId]`, read-modify-write blob pod admin JWT-om, RLS)→auto-verzija (trigger)→toast→in-memory re-render. **c-2** = propagacija (`_propagateToSiblings`): edit zakrpa i sestrinske redove koji dijele kategoriju → `final` (`Object.assign` kopija M1+M2) ostaje u sinku s midtermom. **Živo dokazano authed Playwrightom:** edit `te2M1 demand/0`→marker persistira u bazu I u te2Final→revert vraća oba (prod netaknut 51 red); `content_versions` snapshoti (op=UPDATE, edited_by=Leon = undo+audit). Vježbe se NE diraju (BUG-012).
  **✅ PLAYWRIGHT LOGIN (storageState) — pozitivan admin-put AUTOMATIZIRAN (2026-07-08, `d57c5fd`):** zatvara [[live-login-verifies-crud]] rupu. `playwright.config.js` (dotenv + uvjetni `auth-setup`/`authenticated` projekti kad je `TEST_ADMIN_*` set → default suite netaknut) · `tests/auth.setup.js` (signInWithPassword+is_admin→storageState `tests/.auth/admin.json`, gitignored) · `tests/admin-detect.authed.spec.js` (isAdmin=true + admin vidi edit-gumbe). `npm run test:authed` **3/3 živo**. CI: zaseban `authed` job (`34b3612`, gate-an na repo-secret; ⏳ Leon doda `TEST_ADMIN_EMAIL/PASSWORD`). **Write-testovi svjesno NEautomatizirani** (dijeljena prod baza + append-only audit; nema izoliranog test-DB-a na free tieru → write se provjerava privremenim authed edit-pa-revert specom + MCP, spec se obriše). **⚠️ 22 test-audit-reda (te2) u `content_versions` iz živih proba (6 c-1/c-2 + 4 quiz + 4 fill + 8 learn incl. restore) — bezopasni; brisanje traži izričit OK (auto-mode štiti append-only audit).**
  **▶ F4.4 (proširi tipove) — quiz cigla ✅ GOTOVA + ŽIVO VERIFICIRANA (2026-07-08, `9c2c979`, cache `20260708021017`):** write-helperi generalizirani (`arrayKey`+`applyItem` → flashcard put bit-identičan, quiz bez duplikacije); viewer crta quiz po kategoriji (preview opcija s označenim točnim; quiz-only kategorije sad vidljive); **quiz-editor** `#adminQuizModal` = pitanje + **dinamičke opcije 2–6** (dodaj/obriši + radio „točan"), validacija po JSON Schemi, `image`/`imageAlt` netaknuti. i18n `admin.quiz*` (en/hr); CSS `.admin-quiz-*`. Gate: statika 0/0 + **Playwright 60/0** + **`test:authed` 4/4**. **Živa provjera (authed Playwright + Supabase MCP):** edit `te2 fundamentals/quiz[0]` (pitanje+`correct`) → persistirao u te2M1 I te2Final (propagacija) → revert vratio oba (prod 51 red netaknut).
  **▶ F4.4-fill ✅ GOTOVA + ŽIVO VERIFICIRANA (2026-07-08, cache `20260708024031`):** najjednostavniji tip (`sentence`+`answer`) na istom generaliziranom pipelineu (`arrayKey='fillBlanks'`); viewer reuse `.admin-card-*` (**0 novog CSS-a**); **fill-editor** `#adminFillModal` = rečenica + odgovor, validacija po schemi (**rečenica mora sadržavati `_______`**; oba neprazna); `hint` netaknut. i18n `admin.fill*`. Gate: statika 0/0 + **Playwright 64/0** + **`test:authed` 5/5**. Živa provjera: edit `te2 fundamentals/fillBlanks[0]` → persistiralo u M1 I Final → revert (prod 51 red netaknut).
  **▶ F4.4-learn ✅ GOTOVA + ŽIVO VERIFICIRANA (2026-07-08, cache `20260708060435`):** learn je **jedan objekt po kategoriji** (`{title?, content, image?}`, NE niz) → vlastiti object-put (`_patchLearnObj`/`_propagateLearnToSiblings`; array-put netaknut); viewer `.admin-card--learn` (čist izvadak preko `_adminExcerpt`); **learn-editor** `#adminLearnModal` (širi+monospace) = naslov + **sirovi HTML** (sprema doslovno, KaTeX/HTML očuvani); `image` netaknut. **🐛 živi verifikator našao: `_saveLearn` je trimao `content`** (learn HTML ima namjernu uvlaku → trim kvari formatiranje+revert) → **popravljeno (content se ne trima)**. Gate: statika 0/0 + **Playwright 68/0** + **`test:authed` 6/6**. Živa provjera: edit `te2 fundamentals.learn` (naslov+4KB HTML) → M1 I Final → revert bit-točan (sha1==izvor); pali run ostavio marker → odmah vraćeno na kanonsku JSON vrijednost (prod 51 red netaknut). **⬜ Dalje: kategorije (dodaj/obriši/presloži)** → F4.5 export+dry-run → F4.6 stupnjeviti flip.
  **🧭 ODLUKA (korisnik 2026-07-06): DOVRŠITI F4 (i platformu) PRIJE HR-ekspanzije.** Rast = **HR 1. godina × 3 smjera FMTU (MUH/MUT/MOR)**, dijele „vezne" predmete → arhitektura identiteta u **ADR-022 / `docs/CATALOG_ARCHITECTURE.md`** (placement≠sadržaj, prefiks fakulteta, dijeli-unutar-fakulteta, verify-gate). Timeline: ~3 mj do godine, kolokviji ~4. mj → runway. [[foundation-pivot]] [[hrv-program]]
...
**`CRUD_PLAN`** (▶ AKTIVNO: F4 Admin CRUD — brick-slijed F4.1–F4.6; ADR-021; F4.1/F4.2 gotove+verificirane) ·
**`CATALOG_ARCHITECTURE`** (identitet predmeta preko programa/fakulteta: placement≠sadržaj, prefiks fakulteta, dijeli-unutar-fakulteta, verify-gate; ADR-022; za HR-ekspanziju nakon F4).
