...
- **🎯 SLJEDEĆI RAD: DOVRŠI FAZU „MATERIJAL OD NULE DO UČENJA".** Aktivni spec = **`docs/plan/MATERIJAL_FAZA.md`**, definicija + kriteriji prihvaćanja = **`docs/product/UGC_SPEC.md`**. Tek **iza** toga: frontend redizajn, pa objava/dijeljenje + MCP.
  > ⚠️ **Ranija tvrdnja „sljedeći rad = frontend redizajn" bila je POGREŠNA** (Leon 2026-08-07: *„nismo još završili UGC da se gradi od nule"*). Faza je bila proglašena gotovom po **odčekiranoj tablici cigli**, a ne po cilju iz vizije. **Faza je gotova kad korisnik može napraviti stvar** — mjeri se po `UGC_SPEC §2`, nikad po tablici. [[follow-recorded-plan-dont-reopen]]
- **📊 FAZA: 4½ od 5 kriterija** (grana `docs/stage-a`, `main` NETAKNUT, sve zeleno):
  **M1 ✅** svi modovi u praznom materijalu (`presentModes` je crtao mod samo za NEPRAZAN niz → prva kartica se nije mogla dodati nikad; uređivači su radili, bili su nedostupni) · **M2 ✅** učenje iz vlastitog materijala (materijal se registrira kao **sintetički predmet** `node:<uuid>` u `subjectDataMap` → napredak/analitika/profil/sync rade bez izmjene; dirane 2 funkcije u `navigation.js`) · **M3a ✅** boja **bloka** = akcent `#rrggbb` (`lb-tint` + `--lb-acc`, emitira se samo nakon `^#[0-9a-fA-F]{6}$`; nasljeđivanje je besplatno kroz CSS-kaskadu) · **M4 ✅** sučelje prestalo obećavati (AI-gumb → „Tvoj AI / USKORO") · **M3b ⬜ = JEDINI PREOSTALI KRITERIJ** = boja kartice/pitanja/dopune (tri zasebna prikazivača `flashcards/quiz/fill-blanks` + Studio-paneli — druga površina; sve tri sheme su `additionalProperties: false` pa `color` ne može doći prešutno, a `accent` def iz M3a već postoji).
  **+ M5 (nov nalaz iz Leonovog živog pregleda): DULJINA KARTICE.** Izmjereno na 5379 kartica: pitanja **0** preko 200 (nisu problem) · odgovori **46,2 %** preko 200 · **48 preko 500** (25 jedinstvenih). Tvrdo ograničenje na 200 srušilo bi pola kataloga → **Leonova odluka: strop 500**, u DVA koraka: **M5a** vođenje u editoru (brojač · upozorenje 200 · **tvrda blokada 500**) — ide prvo jer odmah zaustavlja rast; **M5b** skratiti 25 zatečenih pa **tek tada** `maxLength: 500` u shemi (obrnuto = `validate:schema` crven = CI blokiran).
- **✅ LEON JE PREGLEDAO UŽIVO** (preview `studymaster-git-docs-stage-a-…`, prijava **vlastitim PROD-računom** — preview gađa PROD bazu jer je ključ tvrd u `js/auth.js`): *„pregledao sam sve za sada izgleda jako dobro"*. **Staging-override (`localStorage 'sokrat-supabase-override'`) je bio slijepa ulica** — kredencijali staginga na stranici koja gađa PROD = „wrong email or password". **Pouka: za živi pregled ne treba staging, Leonov pravi račun je jednostavniji i vjerniji.** ⚠️ **Ali NIŠTA nije na produkciji** — sve stoji na grani, `main` = `187c646`.
- **⏳ GRANE IZVAN `main`-a:** `docs/stage-a` (cijela faza + reorganizacija dokumentacije; **sadrži `docs/reorg` kao pretka** → ta se grana smije obrisati) · Sašine `content/entrepreneurship-hr` (3) i `content/ebusiness-hr` (1) — **obje diraju `data/catalog.js` i cache-tokene → druga po redu će konfliktirati** (rebase + `npm run bump`).
...
- **NA PRODU (`main`=`a9bf52b`; Vercel `dpl_Coqp…` READY target=production):** 22 predmeta (17 EN u Supabase + 5 HR file-served) · auth+cloud-sync+profil+GDPR · Studio editor · **osobni UGC-graditelj „Moji materijali" (F5, od 2026-08-06)** · **rizik-sprint 7/7** (keep-alive cron protiv free-tier sleepa · supabase-js pin+SRI · backup · load-probe 90/90) · GA4+Sentry (consent) · SW offline · i18n HR/EN · CI+deploy-guard. Tablica predmeta: `docs/subjects/README.md`.
