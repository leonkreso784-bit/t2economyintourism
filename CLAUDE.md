...
- **🎯 AKTIVNA FAZA: „Mjera i zaborav"** (Leon, 2026-08-08) — spec `docs/plan/MJERA_I_ZABORAV.md`, grana `feature/mjera-i-zaborav`, **NIJE deployano**. Zatvara dva 🔥 duga prije redizajna: **strop duljine kartice** (200 upozorenje / **500 tvrda blokada**; politika = `js/card-limits.js`, čitaju je editor I `validate:content`) + **self-service brisanje računa** (Edge Function `delete-account`). ⛔ **Blokada:** puni test brisanja traži `STAGING_SUPABASE_SERVICE_KEY` u `.env` — **samo Leon** ga može dodati; dotad T4/T5 preskaču. Sekvenca dalje: **frontend redizajn** (uklj. „akcent = CIJELA kartica") → **objava/dijeljenje** (doseg presuđen: **link s tajnim tokenom, bez javne biblioteke**) → **MCP**. ⚠️ **Matura = IZBAČENA.**
...
> Puni tekst i sve starije: **`docs/records/DECISIONS.md` (ADR-001…027)**.
